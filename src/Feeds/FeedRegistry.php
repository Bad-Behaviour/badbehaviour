...
class FeedRegistry implements FeedProviderInterface
{
	...
	{
		// === Google (all feeds via single multi-feed adapter) ===
		// This single instance fetches all 5 Google feeds and returns
		// ranges for: googlebot, google_user_triggered, google_user_triggered_agents
		$this->feeds['google'] = new CachedFeedDecorator(
			new GoogleJsonFeed($cache),
			$cache
		);
