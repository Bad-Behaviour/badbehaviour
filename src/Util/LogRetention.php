...
final class LogRetention
{
    ...
    public function do_cleanup(): RetentionResult
    {
        ...
        // === Phase 1: Probe table to discover newest date + total count ===
        ...
        // === Phase 2: Decide what we're deleting by ===
        $cutoff = $this->compute_cutoff($newest, $max_age_days);
        $limit_by = 'age';
        $rows_deleted = 0;
        $iterations = 0;

        // If max_rows cap is set and we'd exceed it, switch to row-count mode.
        ...
        $safety_iterations = 0;
        $max_iterations = 1000;  // hard ceiling, independent of budget
        ...
        try {
            do {
                ...
                if (!$ok) {
                	// Adapter's query() already logged via ErrorReporter.
                	// Route through finalize() so the error string is
                	// captured in RetentionResult for caller diagnostics.
                ...
                // Real row count from the adapter (when it exposes one),
                // else fall back to chunk_size heuristic. An adapter that
                // returns 0 affected rows tells us the table is drained
                // — break out instead of spinning.
                $affected = method_exists($this->adapter, 'lastQueryAffectedRows')
                ? (int)$this->adapter->lastQueryAffectedRows()
                : $this->chunk_size;
        ...
        // === Phase 4: Record last-run timestamp (best-effort) ===
        if ($this->cache !== null && $rows_deleted > 0) {
            try {
                $this->cache->set(
        ...
        $result = new RetentionResult(
            success: $iterations > 0 && $rows_deleted > 0,
