...
class SqliteTestAdapter implements AdapterInterface, CacheInterface
{
    ...
    {
        return ['log_table' => $this->tableName];
    }
    ...
    public function probe_log_table(string $table): array
    {
        try {
            ...
            return ['newest' => $newest, 'total' => $total, 'error' => null];
        } catch (\Throwable $e) {
            return ['newest' => null, 'total' => 0, 'error' => $e->getMessage()];
        }
    }
