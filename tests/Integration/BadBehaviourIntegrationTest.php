...
class BadBehaviourIntegrationTest extends TestCase
{
	...
	{
		$result = $this->bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0.0.0 Safari/537.36', '192.0.2.1', 'GET', '/'));
		$this->assertTrue($result->is_allowed());
	}

	public function test_curl_allowed(): void
	{
		$result = $this->bb->run_test_package(RequestPackage::create_for_test('curl/8.18.0', '192.0.2.1', 'POST', '/api/comment', [
	...
	{
		$result = $this->bb->run_test_package(RequestPackage::create_for_test('Wget/1.21.4', '192.0.2.1', 'GET', '/page'));
		$this->assertTrue($result->is_allowed());
	}

	public function test_json_ajax_preview_allowed(): void
	{
		$result = $this->bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0 Chrome/120.0.0.0', '192.0.2.1', 'POST', '/api/preview', [
	...
	{
		$result = $this->bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0 Chrome/120.0.0.0', '192.0.2.1', 'POST', '/upload', [
	...
	{
		$result = $this->bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0', '192.0.2.1', 'GET', '/?id=1+union+select+1'));
	...
	{
		$result = $this->bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0', '192.0.2.1', 'GET', '/?q=<script>alert(1)</script>'));
		$this->assertFalse($result->is_allowed());
		$this->assertEquals(ResultCode::BLOCKED_ATTACK_PATTERN, $result->code);
	}
	...
	{
		$result = $this->bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0', '192.0.2.1', 'POST', '/comment', [
	...
	public function test_json_body_sqli_allowed(): void
	{
		...
		$result = $this->bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0', '192.0.2.1', 'POST', '/api/comment', [
			'Content-Type' => 'application/json',
			'Accept' => 'application/json'
		], [
	...
	{
		$result = $this->bb->run_test_package(RequestPackage::create_for_test('sqlmap/1.0', '192.0.2.1'));
		$this->assertFalse($result->is_allowed());
	...
	public function test_fake_msie_blocked(): void
	{
		...
		$result = $this->bb->run_test_package(RequestPackage::create_for_test('Mozilla/4.0 (compatible; MSIE 6.0; Windows XP)', // Fake: uses marketing name
		'192.0.2.1', 'GET', '/'));
		$this->assertFalse($result->is_allowed());
		$this->assertEquals(ResultCode::BLOCKED_MALICIOUS_UA, $result->code);
	}
	...
	public function test_verified_googlebot_bypasses_all(): void
	{
		...
		$result = $this->bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)', '66.249.64.1', // Google IP range
		'GET', '/'));
	...
	public function test_rate_limiting(): void
	{
		...
		for ($i = 0; $i < 5; $i ++) {
			$result = $bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0', '192.0.2.100', 'GET', '/'));
