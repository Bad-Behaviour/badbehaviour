...
class PerformanceBenchmarkTest extends TestCase
{
	...
	public function test_static_resource_fast_path(): void
	{
		...
		for ($i = 0; $i < $iterations; $i ++) {
			$this->bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0 Chrome/120', '203.0.113.1', 'GET', '/wp-content/themes/style.css'));
		}
		...
		$this->assertLessThan(0.5, $per_iter, "Static resource fast-path too slow: {$per_iter} ms (target: <0.5 ms)");
	}
	...
	public function test_empty_ua_fast_path(): void
	{
		...
		for ($i = 0; $i < $iterations; $i ++) {
			$this->bb->run_test_package(RequestPackage::create_for_test('', '198.51.100.43', 'GET', '/'));
		}
	...
	public function test_legitimate_browser(): void
	{
		...
		for ($i = 0; $i < $iterations; $i ++) {
			$this->bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0.0.0', '203.0.113.1', 'GET', '/about'));
		}
	...
	public function test_known_search_engine(): void
	{
		...
		for ($i = 0; $i < $iterations; $i ++) {
			$this->bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)', '66.249.66.1', 'GET', '/sitemap.xml'));
		}
