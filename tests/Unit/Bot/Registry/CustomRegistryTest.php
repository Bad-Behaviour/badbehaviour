...
final class CustomRegistryTest extends TestCase
{
    ...
    private function full_bot(string $id = 'full_bot'): array
    {
        return [
            'name'                => 'Full Bot',
            'user_agent_patterns' => ['FullBot', 'FullBot/2.0'],
            'host_patterns'       => ['fullbot.example.com'],
            'ip_ranges'           => ['192.0.2.0/24'],
            'verify_dns'          => true,
            'dns_suffixes'        => ['fullbot.example.com'],
            'category'            => 'ai_crawler',
    ...
    public function test_full_bot_loads_cleanly(): void
    {
        ...
        $this->assertSame(['FullBot', 'FullBot/2.0'], $def->user_agent_patterns);
    ...
    {
        $a = new CustomRegistry(['bot_a' => $this->valid_bot('bot_a')]);
        $b = CustomRegistry::from_array(['bot_a' => $this->valid_bot('bot_a')]);
    ...
    public function test_default_action_defaults_to_allow_when_omitted(): void
    {
        $registry = new CustomRegistry([
            'no_action' => [
                'name'                => 'No Action',
                'user_agent_patterns' => ['NoActionBot'],
                'category'            => 'monitoring',
            ],
        ]);
        ...
        $this->assertSame(BotAction::ALLOW, $def->default_action,
            'Omitted default_action must default to ALLOW');
    }
    ...
    {
        $registry = new CustomRegistry([
            'bot' => array_merge($this->valid_bot(), ['name' => '']),
        ]);
    ...
    {
        $registry = new CustomRegistry([
            'bot' => array_merge($this->valid_bot(), ['name' => ['nope']]),
        ]);
    ...
    {
        $registry = new CustomRegistry([
            'bot' => array_merge($this->valid_bot(), ['user_agent_patterns' => []]),
        ]);
    ...
    {
        $registry = new CustomRegistry([
            'bot' => array_merge($this->valid_bot(), ['category' => 42]),
        ]);
    ...
    public function test_user_agent_patterns_shorter_than_three_chars_is_rejected(string $pattern): void
    {
        $registry = new CustomRegistry([
            'bot' => array_merge(
                $this->valid_bot(),
                ['user_agent_patterns' => [$pattern]]
            ),
        ]);

        $this->assertTrue($registry->has_errors(),
            "UA pattern '{$pattern}' must be rejected");
        $this->assertFalse($registry->has('bot'));
    }
    ...
    public function test_non_string_pattern_entry_is_rejected(): void
    {
        $registry = new CustomRegistry([
            'bot' => array_merge(
                $this->valid_bot(),
                ['user_agent_patterns' => ['Valid', 123, 'AlsoValid']]
            ),
        ]);
    ...
    public function test_at_least_one_valid_pattern_in_mixed_list_rejects_whole_entry(): void
    {
        $registry = new CustomRegistry([
            'bot' => array_merge(
                $this->valid_bot(),
                ['user_agent_patterns' => ['GoodBot', 'B']]
            ),
        ]);

        $this->assertTrue($registry->has_errors(),
            'One invalid pattern must drop the entire bot definition');
        $this->assertFalse($registry->has('bot'));
    }
    ...
    {
        $registry = new CustomRegistry([
            'bot' => array_merge($this->valid_bot(), ['category' => $category]),
        ]);
    ...
    {
        return [
            'not_an_enum'      => ['not_a_real_category'],
            'capitalized'      => ['Search_Engine'],
            'with_typo'        => ['serch_engine'],
            'empty_string'     => [''],
        ];
    }
    ...
    public function test_all_bot_category_values_are_accepted(): void
    {
        foreach (BotCategory::cases() as $cat) {
            ...
            $this->assertFalse(
                $registry->has_errors(),
                "BotCategory::{$cat->name} ('{$cat->value}') must be a valid category value"
            );
    ...
    {
        $registry = new CustomRegistry([
            'bot' => array_merge($this->valid_bot(), ['default_action' => $action]),
        ]);
    ...
    public function test_all_bot_action_values_are_accepted(): void
    {
        foreach (BotAction::cases() as $action) {
            ...
            $this->assertFalse(
                $registry->has_errors(),
                "BotAction::{$action->name} ('{$action->value}') must be a valid default_action"
            );
    ...
    public function test_valid_ipv4_cidrs_are_accepted(): void
    {
        $registry = new CustomRegistry([
            'bot' => array_merge(
                $this->valid_bot(),
                ['ip_ranges' => ['10.0.0.0/8', '192.0.2.0/24', '172.16.0.0/12']]
            ),
        ]);
    ...
    public static function invalidCidrProvider(): array
    {
        return [
            ...
            'no_mask'           => ['10.0.0.0'],           // no '/' at all
            'just_ip'           => ['192.168.1.1'],        // no '/' at all
            'non_numeric_mask'  => ['10.0.0.0/abc'],       // non-digits after '/'
            'negative_mask'     => ['10.0.0.0/-1'],        // '-' not in [\d]
            'empty_mask'        => ['10.0.0.0/'],          // no digits after '/'
            'comma_separator'   => ['10,0,0,0/24'],        // ',' not in [\d\.:a-fA-F]
            'space_in_ip'       => ['10.0.0. 0/24'],       // ' ' not in [\d\.:a-fA-F]
            'slash_at_start'    => ['/24'],                 // no IP part
            'multiple_slashes'  => ['10.0.0.0/24/extra'],  // extra '/' breaks $
    ...
    public function test_dns_suffixes_array_of_strings_is_accepted(): void
    {
        $registry = new CustomRegistry([
            'bot' => array_merge(
                $this->valid_bot(),
                ['dns_suffixes' => ['bot.example.com', 'cdn.example.net']]
            ),
        ]);
    ...
    {
        $registry = new CustomRegistry([
            'bot' => array_merge($this->valid_bot(), ['robots_txt_token' => 'MyBot']),
        ]);
    ...
    {
        $registry = new CustomRegistry([
            'bot' => array_merge($this->valid_bot(), ['robots_txt_token' => null]),
        ]);
    ...
    {
        $registry = new CustomRegistry([
            'bot' => array_merge($this->valid_bot(), ['robots_txt_token' => 123]),
        ]);
    ...
    public function test_host_patterns_array_is_accepted(): void
    {
        $registry = new CustomRegistry([
            'bot' => array_merge(
                $this->valid_bot(),
                ['host_patterns' => ['bot.example.com', 'cdn.example.net']]
            ),
        ]);
    ...
    public function test_invalid_entries_do_not_block_valid_entries(): void
    {
        $registry = new CustomRegistry([
            ...
            'missing_name'    => [
                'user_agent_patterns' => ['MissingName'],
                'category'            => 'search_engine',
            ],
            'bad_category'    => array_merge(
                $this->valid_bot('bad_category'),
                ['category' => 'not_real']
            ),
            'another_good'    => $this->valid_bot('another_good'),
            'too_short_ua'    => array_merge(
                $this->valid_bot('too_short_ua'),
                ['user_agent_patterns' => ['X']]
            ),
        ]);
    ...
    {
        $registry = new CustomRegistry();
        $def = new BotDefinition(
            id: 'added_bot',
            name: 'Added Bot',
            user_agent_patterns: ['AddedBot'],
            host_patterns: [],
            ip_ranges: [],
        ...
        $this->assertFalse($registry->has_errors(),
            'add() must not pollute get_errors() — it does not run validation');
    }
    ...
    public function test_add_bypasses_validation_entirely(): void
    {
        ...
        $this->assertTrue($registry->has('unusual_bot'),
            'add() must accept any BotDefinition without validation');
    }
    ...
    public function test_add_overwrites_existing_bot_with_same_id(): void
    {
        ...
        $override = new BotDefinition(
            id: 'slot',
            name: 'Overridden',
            user_agent_patterns: ['OverriddenBot'],
            host_patterns: [],
            ip_ranges: [],
            category: BotCategory::MONITORING,
        );
    ...
    {
        $registry = new CustomRegistry([
            'unique_token_bot' => array_merge(
                $this->valid_bot('unique_token_bot'),
                ['user_agent_patterns' => ['UniqueMarker77']]
            ),
        ]);
    ...
    {
        $registry = new CustomRegistry([
            'short_token_bot' => array_merge(
                $this->valid_bot('short_token_bot'),
                ['user_agent_patterns' => ['bot']]
            ),
        ]);
        ...
        $this->assertNotContains('short_token_bot', $matches,
            'Tokens shorter than MIN_TOKEN_LENGTH (5) must be filtered out');
    }
    ...
    {
        $registry = new CustomRegistry([
            'a' => array_merge($this->valid_bot('a'), ['category' => 'search_engine']),
            'b' => array_merge($this->valid_bot('b'), ['category' => 'ai_crawler']),
            'c' => array_merge($this->valid_bot('c'), ['category' => 'search_engine']),
        ]);
    ...
    public function test_cloud_infrastructure_category_accessor(): void
    {
        $registry = new CustomRegistry([
            'cf' => array_merge(
                $this->valid_bot('cf'),
                ['category' => 'cloud_infrastructure', 'user_agent_patterns' => ['Cloudflare-Healthcheck']]
            ),
        ]);
    ...
    public function test_residential_crawlers_category_accessor(): void
    {
        $registry = new CustomRegistry([
            'rdp' => array_merge(
                $this->valid_bot('rdp'),
                ['category' => 'residential_proxy', 'user_agent_patterns' => ['BrightData']]
            ),
        ]);
    ...
    public function test_count_reflects_valid_bots_only(): void
    {
        ...
        $this->assertSame(2, $registry->count(),
            'count() counts only successfully validated bots');
    }
