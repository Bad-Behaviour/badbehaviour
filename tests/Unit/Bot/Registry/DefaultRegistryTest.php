...
final class DefaultRegistryTest extends TestCase
{
    ...
    public function test_default_registry_ships_a_substantial_number_of_bots(): void
    {
        ...
        $this->assertGreaterThanOrEqual(50, $count,
            "DefaultRegistry should ship at least 50 bots (got {$count})");
    }
    ...
    public function test_every_bot_has_at_least_one_user_agent_pattern(): void
    {
        $registry = $this->get_default();
        foreach ($registry->all() as $id => $def) {
            $this->assertNotEmpty($def->user_agent_patterns,
                "Bot '{$id}' has no user_agent_patterns");
        }
    }
        ...
        foreach ($registry->all() as $id => $def) {
            foreach ($def->user_agent_patterns as $i => $pattern) {
                $this->assertIsString($pattern,
                    "Bot '{$id}' has non-string UA pattern at index {$i}");
            }
        }
    }
    ...
    public function test_every_user_agent_pattern_is_non_empty(): void
    {
        ...
        foreach ($registry->all() as $id => $def) {
            foreach ($def->user_agent_patterns as $i => $pattern) {
                $this->assertNotEmpty(trim((string)$pattern),
                    "Bot '{$id}' has empty UA pattern at index {$i}");
            }
        }
    }
    ...
    public function test_every_user_agent_pattern_meets_minimum_length(): void
    {
        ...
        foreach ($registry->all() as $id => $def) {
            ...
            $this->assertTrue($has_long_enough,
                "Bot '{$id}' has no UA patterns ≥{$min_useful_length} chars");
        }
    }
    ...
    public function test_every_bot_has_a_valid_category(): void
    {
        ...
        foreach ($registry->all() as $id => $def) {
            $this->assertContains($def->category->value, $valid_categories,
                "Bot '{$id}' has invalid category '{$def->category->value}'");
        }
    }
    ...
    public function test_every_bot_category_is_a_botcategory_enum_instance(): void
    {
        $registry = $this->get_default();
        foreach ($registry->all() as $id => $def) {
            $this->assertInstanceOf(BotCategory::class, $def->category,
                "Bot '{$id}' category is not a BotCategory enum");
        }
    }
    ...
    public function test_all_category_accessors_return_only_that_category(): void
    {
        $registry = $this->get_default();
        $accessors = [
            'search_engines', 'ai_crawlers', 'social_crawlers',
            'seo_crawlers', 'archive_crawlers', 'monitoring',
            'feed_readers', 'shopping_crawlers', 'cloud_infrastructure',
            'security_scanners', 'residential_crawlers',
        ];
        ...
        foreach ($registry->all() as $id => $def) {
            ...
            $this->assertSame(1, $appearances,
                "Bot '{$id}' appears in {$appearances} category accessors (must be exactly 1)");
        }
    }
        ...
        foreach ($registry->all() as $id => $def) {
            foreach ($def->ip_ranges as $i => $cidr) {
                $this->assertIsString($cidr,
                    "Bot '{$id}' has non-string IP range at index {$i}");
                $this->assertNotEmpty(trim($cidr),
                    "Bot '{$id}' has empty IP range at index {$i}");
            }
        }
    }
    ...
    public function test_every_ip_range_is_parseable_by_iputil(): void
    {
        ...
        $this->assertEmpty($failures,
            "DefaultRegistry contains invalid CIDRs:\n  " . implode("\n  ", $failures));
    }
    ...
    {
        $registry = $this->get_default();
        $this->assertTrue($registry->has($bot_id),
            "DefaultRegistry must ship cloud_infrastructure bot '{$bot_id}'");
    }
    ...
    public static function expectedCloudBotProvider(): array
    {
        return [
            ...
            'AWS ELB'    => ['aws_elb_health'],
            'GCP LB'     => ['google_cloud_health'],
            'Azure LB'   => ['azure_health'],
    ...
    public function test_cloud_infrastructure_bots_have_ip_ranges(): void
    {
        ...
        foreach ($registry->cloud_infrastructure() as $id => $def) {
            ...
            $this->assertNotEmpty($def->ip_ranges,
                "Cloud bot '{$id}' has no ip_ranges");
        }
    }
    ...
    public function test_cloud_infrastructure_bots_have_default_action_allow(): void
    {
        ...
        foreach ($registry->cloud_infrastructure() as $id => $def) {
            $this->assertSame(BotAction::ALLOW, $def->default_action,
                "Cloud bot '{$id}' has default_action={$def->default_action->value}, expected ALLOW");
        }
    }
    ...
    public function test_every_bot_has_a_default_action_enum(): void
    {
        $registry = $this->get_default();
        foreach ($registry->all() as $id => $def) {
            $this->assertInstanceOf(BotAction::class, $def->default_action,
                "Bot '{$id}' default_action is not a BotAction enum");
        }
    }
    ...
    public function test_every_bot_host_patterns_is_array(): void
    {
        $registry = $this->get_default();
        foreach ($registry->all() as $id => $def) {
            $this->assertIsArray($def->host_patterns,
                "Bot '{$id}' host_patterns must be an array");
        }
    }
    ...
    public function test_every_bot_dns_suffixes_is_array(): void
    {
        $registry = $this->get_default();
        foreach ($registry->all() as $id => $def) {
            $this->assertIsArray($def->dns_suffixes,
                "Bot '{$id}' dns_suffixes must be an array");
        }
    }
    ...
    public function test_verify_dns_true_bots_have_dns_suffixes(): void
    {
        ...
        $this->assertEmpty($violations,
            'Bots with verify_dns=true but empty dns_suffixes: ' . implode(', ', $violations));
    }
    ...
    public function test_robots_txt_token_is_string_or_null(): void
    {
        ...
        foreach ($registry->all() as $id => $def) {
            if ($def->robots_txt_token !== null) {
                $this->assertIsString($def->robots_txt_token,
                    "Bot '{$id}' robots_txt_token must be null or string");
            }
        }
    }
    ...
    public function test_per_category_accessors_partition_the_registry(): void
    {
        ...
        $this->assertSame(
            $registry->count() - 1,
            $total_from_accessors,
            "Sum of per-category counts should equal total - 1 "
            . "('iabot' is in utility_bots(), not in any category accessor)"
        );
    }
    ...
    public function test_expected_bot_is_present(string $bot_id, string $expected_category): void
    {
        $registry = $this->get_default();
        $this->assertTrue($registry->has($bot_id),
            "DefaultRegistry should ship '{$bot_id}'");

        $def = $registry->get($bot_id);
        $this->assertSame($expected_category, $def->category->value,
            "Bot '{$bot_id}' has wrong category '{$def->category->value}' (expected '{$expected_category}')");
    }
    ...
    public static function expectedBotProvider(): array
    {
        return [
            ...
            'yandex'           => ['yandex',           'search_engine'],
            'duckduckgo'       => ['duckduckgo',       'search_engine'],
            'gptbot'           => ['gptbot',           'ai_crawler'],
            'claude'           => ['claude',           'ai_crawler'],
            'perplexity'       => ['perplexity',       'ai_crawler'],
            'google_ai'        => ['google_ai',        'ai_crawler'],
            'meta_ai'          => ['meta_ai',          'ai_crawler'],
            'facebook'         => ['facebook',         'social_crawler'],
            'twitter'          => ['twitter',          'social_crawler'],
            'linkedin'         => ['linkedin',         'social_crawler'],
            'semrush'          => ['semrush',          'seo_crawler'],
            'ahrefs'           => ['ahrefs',           'seo_crawler'],
            'uptimerobot'      => ['uptimerobot',      'monitoring'],
            'brightdata'       => ['brightdata',       'residential_proxy'],
        ];
    }
}
