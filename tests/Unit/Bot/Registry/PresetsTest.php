...
final class PresetsTest extends TestCase
{
	...
	private function assert_has_bot(string $bot_id, $registry): void
	{
		$this->assertTrue(
			$registry->has($bot_id),
			"Preset must include bot '{$bot_id}'"
		);
	}
	...
	private function assert_lacks_bot(string $bot_id, $registry): void
	{
		$this->assertFalse(
			$registry->has($bot_id),
			"Preset must NOT include bot '{$bot_id}'"
		);
	}
	...
	public function test_available_constant_lists_all_known_presets(): void
	{
		...
		$this->assertSame($expected, Presets::AVAILABLE,
			'AVAILABLE must list every preset name load() accepts');
	}
	...
	public function test_is_valid_returns_true_for_each_available_preset(): void
	{
		foreach (Presets::AVAILABLE as $name) {
			$this->assertTrue(
				Presets::is_valid($name),
				"is_valid('{$name}') must return true (it is in AVAILABLE)"
			);
		}
	}
	...
	{
		$invalid = ['', 'nonexistent', 'Minimal', 'FULL', 'minimal ', 'minimal;'];

		foreach ($invalid as $name) {
			$this->assertFalse(
				Presets::is_valid($name),
				"is_valid('{$name}') must return false"
			);
		}
	}
	...
	public function test_load_returns_a_registry_implementation(): void
	{
		foreach (Presets::AVAILABLE as $name) {
			...
			$this->assertInstanceOf(
				\BadBehaviour\Bot\RegistryInterface::class,
				$registry,
				"load('{$name}') must return a RegistryInterface"
			);
		}
	}
	...
	public function test_full_preset_returns_full_default_registry(): void
	{
		...
		$this->assertSame($default->count(), $registry->count(),
			'full preset must have same bot count as DefaultRegistry');
	...
	public function test_minimal_preset_includes_all_cloud_infrastructure(): void
	{
		...
		foreach ($cloud_ids as $bot_id) {
			$this->assert_has_bot(
				$bot_id,
				$registry,
				"CRITICAL: minimal preset must include cloud_infrastructure bot '{$bot_id}' "
				. '(blocking CDN/LB health probes takes origin offline)'
			);
		}
	}
	...
	public function test_minimal_preset_excludes_obscure_bots(): void
	{
		...
		foreach (['coccoc', 'marginalia', 'stract', 'wiby', 'centrum'] as $rare) {
			if (!$default->has($rare)) {
				continue;
			}
		...
		$this->assertGreaterThanOrEqual(20, $registry->count(),
			'minimal preset must still cover the ~30 most common bots (sanity lower bound)');
		$this->assertLessThanOrEqual(60, $registry->count(),
			'minimal preset must not bloat (sanity upper bound)');
	}
	...
	public function test_verified_only_excludes_unverifiable_bots(): void
	{
		...
		foreach ($registry->all() as $id => $def) {
			...
			$this->assertTrue(
				$verified_capable,
				"verified-only must include only bots that have DNS verification or IP ranges. "
				. "Bot '{$id}' has neither (verify_dns=" . var_export($def->verify_dns, true)
				. ', ip_ranges=' . count($def->ip_ranges) . ")"
					);
		}
	}
	...
	public function test_no_ai_excludes_ai_crawlers(): void
	{
		...
		foreach ($ai_bot_ids as $bot_id) {
			$this->assert_lacks_bot(
				$bot_id,
				$registry,
				"no-ai preset must NOT include AI crawler '{$bot_id}'"
			);
		}
	}
	...
	public function test_no_ai_ai_crawlers_accessor_returns_empty(): void
	{
		...
		$this->assertSame(
			...
			"no-ai preset's ai_crawlers() must return an empty array"
		);
	}
	...
	public function test_no_seo_excludes_seo_crawlers(): void
	{
		...
		foreach ($seo_bot_ids as $bot_id) {
			$this->assert_lacks_bot(
				$bot_id,
				$registry,
				"no-seo preset must NOT include SEO crawler '{$bot_id}'"
			);
		}
	}
	...
	public function test_eu_only_includes_cloud_infrastructure(): void
	{
		...
		$cloud_ids = array_keys($default->cloud_infrastructure());
		foreach ($cloud_ids as $bot_id) {
			$this->assert_has_bot($bot_id, $registry,
				"eu-only must include cloud bot '{$bot_id}' (availability guarantee)");
		}
	}
	...
	public function test_human_only_returns_empty_registry(): void
	{
		...
		$this->assertInstanceOf(EmptyRegistry::class, $registry,
			"human-only must be an EmptyRegistry (no bots recognized)");
	...
	public function test_human_only_singleton_returns_same_instance(): void
	{
		...
		$this->assertSame($a, $b,
			'human-only preset should return the singleton EmptyRegistry instance');
	}
	...
	public function test_preset_includes_cloud_infrastructure_when_it_has_any_bots(string $preset): void
	{
		...
		foreach ($default->cloud_infrastructure() as $def) {
			$this->assert_has_bot(
				$def->id,
				$registry,
				"CRITICAL AVAILABILITY BUG: preset '{$preset}' includes bots but is missing "
				. "cloud_infrastructure bot '{$def->id}'. Blocking CDN/LB health probes takes "
				. 'the origin offline. Re-add the bot to the preset.'
			);
		}
	}
	...
	public function test_full_covers_all_categories(): void
	{
		...
		foreach ($categories_with_bots as $cat) {
			...
			$this->assertGreaterThan(
				0,
				count($registry->{$this->category_method($cat)}()),
				"full preset must include bots from category {$cat->value}"
			);
		}
	}
	...
	public function test_load_accepts_custom_base_registry(): void
	{
		...
		$this->assertSame($custom_base, $registry,
			"'full' preset with explicit base should return the base as-is");
	}
	...
	public function test_minimal_with_custom_base_filters_from_that_base(): void
	{
		...
		$base = new \BadBehaviour\Bot\Registry\InMemoryRegistry([
			'custom_only_bot_a' => new \BadBehaviour\Bot\BotDefinition(
				id: 'custom_only_bot_a',
				name: 'A',
				user_agent_patterns: ['CustomOnlyA'],
				host_patterns: [],
				ip_ranges: [],
				category: BotCategory::MONITORING,
			),
			'googlebot' => new \BadBehaviour\Bot\BotDefinition(
				id: 'googlebot',
				name: 'GB',
