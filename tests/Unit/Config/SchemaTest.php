...
class SchemaTest extends TestCase
{
    ...
    public function test_flatten_preserves_lists_at_parent_level(): void
    {
        ...
        $this->assertSame(['GPTBot', 'ClaudeBot'], $flat['ai_crawlers.allowed']);
    }
    ...
    public function test_unknown_keys_empty_for_valid_config(): void
    {
        ...
        $this->assertEmpty(
            $unknown,
            'get_defaults() flattened must produce zero unknown keys. Got: '
            . implode(', ', $unknown)
        );
    }
    ...
    public function test_unknown_keys_handles_internal_markers(): void
    {
        ...
        $this->assertContains(
            '_safe_mode',
            $unknown,
            '_safe_mode is not in KEY_MAP (correctly — it is internal)'
        );
    }
    ...
    public function test_property_for_returns_null_for_collapsible(): void
    {
        ...
        $this->assertNull(
            Schema::property_for('bot_categories.blocked'),
            'bot_categories.blocked is collapsible (collapses into bot_categories array)'
        );
        $this->assertNull(
            Schema::property_for('bot_categories.allowed'),
            'bot_categories.allowed is collapsible'
        );

        // rate_limits.* sub-keys are collapsible (into rate_limits array)
        $this->assertNull(
            Schema::property_for('rate_limits.global.requests'),
            'rate_limits.global.requests is collapsible'
        );

        // geoip.blocked_* are collapsible
        $this->assertNull(
            Schema::property_for('geoip.blocked_countries'),
            'geoip.blocked_countries is collapsible'
        );
    }
    ...
    public function test_all_default_keys_are_in_schema(): void
    {
        ...
        $injected = ['adapter', 'logger', 'cache', 'geoip', 'log_table'];

        $missing = [];
        foreach (array_keys($defaults_flat) as $dotted) {
            if (in_array($dotted, $injected, true)) continue;
        ...
        $this->assertEmpty(
            $missing,
            'All default keys must be in Schema::KEY_MAP. Missing: '
            . implode(', ', $missing)
        );
    }
    ...
    public function test_strictness_override_keys_in_schema(): void
    {
        ...
        foreach (['monitor-only', 'normal', 'strict'] as $level) {
            ...
            $this->assertEmpty(
                $unknown,
                "strictness_overrides('$level') must use known schema keys. "
    ...
    public function test_safemode_override_keys_in_schema(): void
    {
        ...
        $this->assertEmpty(
            $unknown,
            'SafeMode::overrides() must use known schema keys. '
            . 'Unknown: ' . implode(', ', $unknown)
        );
    }
    ...
    public function test_lists_in_nested_values_are_preserved(): void
    {
        $nested = [
            'dnsbl' => [
                'lists' => ['zen.spamhaus.org', 'bl.spamcop.net'],  // list
            ],
            'blocked_bot_categories' => ['malicious', 'seo_crawler'],  // list at top
        ];
        ...
        $this->assertSame(
            ...
            $flat['dnsbl.lists'],
            'list inside nested section must be preserved as value'
        );
        $this->assertArrayHasKey(
            'blocked_bot_categories',
            $flat,
            'top-level list must stay at parent (not become blocked_bot_categories.0)'
        );
        $this->assertArrayNotHasKey(
            'blocked_bot_categories.0',
            $flat,
            'list elements must NOT become individual dotted keys'
        );
    }
}