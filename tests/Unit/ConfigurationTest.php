...
class ConfigurationTest extends TestCase
{
	...
	public function test_from_array_defaults(): void
	{
		...
		$this->assertEquals([
			...
			'fastly',
			'gcp'
		], $config->dynamic_ip_ranges_feeds);
	...
	public function test_bot_categories_round_trip(): void
	{
		...
		$this->assertEquals([
			'malicious',
			'residential_proxy'
		], $config->blocked_bot_categories);
		$this->assertEquals([
			'social_crawler',
			'ai_crawler'
		], $config->challenge_bot_categories);
		$this->assertEquals([
			'security_scanner'
		], $config->log_only_bot_categories);
		$this->assertEquals([
			'feed_reader',
			'archive_crawler'
		], $config->allowed_bot_categories);
	 ...
	 * defaults after from_array([]).
	 * The empty-config path must not
	 * wipe the array to [].
	 */
	public function test_skip_static_extensions_survives_coercion(): void
	{
		...
		$this->assertIsArray($config->skip_static_extensions, 'skip_static_extensions must be an array');
		$this->assertNotEmpty($config->skip_static_extensions, 'skip_static_extensions must not be empty (defaults must flow through)');
	}
	...
	public function test_minimal_user_config_preserves_static_skip(): void
	{
		...
		$this->assertNotEmpty($config->skip_static_extensions, 'minimal config → skip_static_extensions must be non-empty');
		$this->assertContains('js', $config->skip_static_extensions, "skip_static_extensions must contain 'js' — otherwise .js requests " . "flood the log with verbose=true (the original WackoWiki bug)");
		$this->assertContains('css', $config->skip_static_extensions, "skip_static_extensions must contain 'css'");
		$this->assertContains('png', $config->skip_static_extensions, "skip_static_extensions must contain 'png'");
		$this->assertContains('svg', $config->skip_static_extensions, "skip_static_extensions must contain 'svg'");
		$this->assertContains('woff', $config->skip_static_extensions, "skip_static_extensions must contain 'woff'");
		$this->assertGreaterThanOrEqual(10, count($config->skip_static_extensions), 'skip_static_extensions must have full default set');

		// Same checks for skip_static_paths
		$this->assertNotEmpty($config->skip_static_paths, 'minimal config → skip_static_paths must be non-empty');
		$this->assertContains('/static/', $config->skip_static_paths, "skip_static_paths must contain '/static/'");
	}
	...
	public function test_explicit_empty_skip_extensions_respects_user_intent(): void
	{
		...
		$this->assertSame([], $config->skip_static_extensions, 'explicit [] in user config must wipe defaults (user intent)');
		$this->assertSame([], $config->skip_static_paths, 'explicit [] in user config must wipe defaults (user intent)');
	}

	/**
	 * Partial override: user sets only skip_extensions, skip_paths must
	 * inherit defaults.
	 * Verifies the array merge handles nested arrays
	...
	public function test_partial_override_preserves_other_array_defaults(): void
	{
		...
		$this->assertSame([
			'custom_ext'
		], $config->skip_static_extensions, 'user override replaces skip_static_extensions entirely');
		$this->assertNotEmpty($config->skip_static_paths, 'partial override preserves skip_static_paths defaults');
	}
	...
	public function test_all_array_constructor_parameters_survive_coercion(): void
	{
		...
		foreach ($constructor->getParameters() as $param) {
			...
			if ($type instanceof ReflectionUnionType) {
				foreach ($type->getTypes() as $namedType) {
					if ($namedType instanceof ReflectionNamedType && $namedType->getName() === 'array') {
						$array_params[] = $param->getName();
						break;
					}
				}
		...
		$this->assertNotEmpty($array_params, 'Configuration must have at least one array parameter');
		...
		foreach ($array_params as $param) {
			...
			$this->assertIsArray($value, "$param must be an array after from_array([]) " . "(got " . get_debug_type($value) . ')');

			if (in_array($param, $must_be_non_empty, true)) {
				$this->assertNotEmpty($value, "$param must be non-empty (defaults must survive coercion)");
			} elseif (in_array($param, $must_be_empty, true)) {
				$this->assertSame([], $value, "$param must be empty array (not null, not scalar)");
			}
	...
	public function test_specific_array_defaults_preserved(): void
	{
		...
		foreach ($cases as [
			...
			foreach ($must_contain as $required) {
				$this->assertContains($required, $actual, "$prop must contain '$required'");
			}
		}
	}
	 ...
	 * properties.
	 * Without working coercion, INI-loaded configs arrive
	...
	public function test_string_coerces_to_bool(): void
	{
		...
		$this->assertTrue($config->verbose, "string 'true' must coerce to bool true");
		$this->assertTrue($config->logging, "string '1' must coerce to bool true");
	}
	...
	public function test_string_coerces_to_int(): void
	{
		...
		$this->assertSame(25, $config->httpbl_threat, "string '25' must coerce to int 25");
		$this->assertSame(300, $config->dns_verification_timeout_ms, "string '300' must coerce to int 300");
		$this->assertSame(20, $config->head_flood_threshold, "string '20' must coerce to int 20");
	}
	...
	public function test_string_coerces_to_float(): void
	{
		...
		$this->assertSame(0.7, $config->recaptcha_min_score, "string '0.7' must coerce to float 0.7");
	}
	...
	public function test_monitor_only_disables_dns_verification(): void
	{
		...
		$this->assertFalse($config->dns_verification_enabled, "strictness='monitor-only' must disable DNS verification");
	}
	...
	public function test_normal_enables_dns_verification(): void
	{
		...
		$this->assertTrue($config->dns_verification_enabled, "strictness='normal' must enable DNS verification");
	}
	...
	public function test_user_beats_strictness_override(): void
	{
		...
		$this->assertTrue($config->dns_verification_enabled, "user override must beat strictness override");
	}
	...
	public function test_all_strictness_levels_preserve_static_skip(): void
	{
		...
		foreach ([
			...
			$this->assertNotEmpty($config->skip_static_extensions, "strictness='$level' must preserve skip_static_extensions");
			$this->assertContains('js', $config->skip_static_extensions, "strictness='$level' must preserve 'js' in skip_static_extensions");
			$this->assertNotEmpty($config->skip_static_paths, "strictness='$level' must preserve skip_static_paths");
		}
	}
	...
	public function test_typos_in_user_config_are_flagged(): void
	{
		...
		$this->assertArrayHasKey('dynamc_ip_ranges.enabled', $unknown, "typo 'dynamc_ip_ranges.enabled' must be flagged");
		$this->assertArrayHasKey('dns_verfiction.enabled', $unknown, "typo 'dns_verfiction.enabled' must be flagged");
	}
	...
	public function test_known_keys_not_flagged(): void
	{
		...
		$this->assertEmpty($unknown, "valid config keys must not appear in Diagnostics::unknown_keys(). " . "Got: " . implode(', ', array_keys($unknown)));
	}
	...
	public function test_safemode_disables_all_defenses(): void
	{
		...
		foreach ($defense_flags as $prop => $expected) {
			$this->assertSame($expected, $config->{$prop}, "safe-mode must set $prop = " . var_export($expected, true));
		}

		// Logging must remain ON (so traffic is observable)
		$this->assertTrue($config->logging, 'safe-mode must keep logging enabled (for observability)');

		// Adapter-specific log_table injection
		$this->assertSame('test_log_table', $config->log_table, 'safe-mode must inject adapter-specific log_table');
	}

	/**
	 * SafeMode must preserve static-skip defaults.
	 * Without this, every
	...
	public function test_safemode_preserves_static_skip_defaults(): void
	{
		...
		$this->assertNotEmpty($settings['performance']['skip_extensions'], 'safe-mode must preserve performance.skip_extensions defaults');
		$this->assertContains('js', $settings['performance']['skip_extensions'], "safe-mode must preserve 'js' in performance.skip_extensions");
		$this->assertNotEmpty($settings['performance']['skip_paths'], 'safe-mode must preserve performance.skip_paths defaults');
	}

	/**
	 * SafeMode's SAFE_MODE_OVERRIDES must use nested keys matching what
	 * Configuration::from_array() reads.
	 * Flat keys are silently ignored,
	...
	public function test_safemode_overrides_use_nested_keys(): void
	{
		...
		$this->assertEmpty($found_flat, 'SafeMode::overrides() must not use flat keys that from_array() ignores. ' . 'Found: ' . implode(', ', $found_flat));
	}
	...
	public function test_safemode_uses_nested_keys_for_network_features(): void
	{
		...
		$this->assertArrayHasKey('dns_verification', $overrides, 'SafeMode must use nested "dns_verification" key (not flat "dns_verification_enabled")');
		$this->assertArrayHasKey('enabled', $overrides['dns_verification'], 'SafeMode nested "dns_verification" must contain "enabled" sub-key');
		$this->assertFalse($overrides['dns_verification']['enabled'], 'SafeMode must disable dns_verification.enabled');

		$this->assertArrayHasKey('dynamic_ip_ranges', $overrides, 'SafeMode must use nested "dynamic_ip_ranges" key');
		$this->assertFalse($overrides['dynamic_ip_ranges']['enabled'], 'SafeMode must disable dynamic_ip_ranges.enabled');

		$this->assertArrayHasKey('reverse_proxy', $overrides, 'SafeMode must use nested "reverse_proxy" key');
		$this->assertFalse($overrides['reverse_proxy']['enabled'], 'SafeMode must disable reverse_proxy.enabled');
	}
	...
	public function test_schema_covers_all_defaults(): void
	{
		...
		$this->assertEmpty($missing, 'Schema must cover all default config keys. Missing: ' . implode(', ', $missing));
	}
	...
	public function test_strictness_overrides_use_known_schema_keys(): void
	{
		...
		foreach ([
			...
			$this->assertEmpty($unknown, "strictness_overrides('$level') must use only known schema keys. " . 'Unknown: ' . implode(', ', $unknown));
		}
	}
	...
	public function test_safemode_overrides_use_known_schema_keys(): void
	{
		...
		$this->assertEmpty($unknown, "SafeMode::overrides() must use only known schema keys. " . 'Unknown: ' . implode(', ', $unknown));
	}
	...
	public function test_to_array_round_trip_preserves_all_four_bot_categories(): void
	{
		...
		$this->assertEquals([
			'malicious'
		], $round['bot_categories']['blocked'], 'to_array() must round-trip bot_categories.blocked');
		$this->assertEquals([
			'social_crawler'
		], $round['bot_categories']['challenge'], 'to_array() must round-trip bot_categories.challenge');
		$this->assertEquals([
			'security_scanner'
		], $round['bot_categories']['log_only'], 'to_array() must round-trip bot_categories.log_only');
		$this->assertEquals([
			'feed_reader'
		], $round['bot_categories']['allowed'], 'to_array() must round-trip bot_categories.allowed');
	}
	...
	public function test_invalid_strictness_falls_back_to_default(): void
	{
		...
		$this->assertSame('normal', $config->strictness, "invalid strictness must fall back to 'normal' default");
	}
	...
	public function test_httpbl_threat_clamped_to_255(): void
	{
		...
		$this->assertSame(255, $config->httpbl_threat, 'httpbl_threat must be clamped to 255 max');
	}
	...
	public function test_httpbl_threat_clamped_to_zero(): void
	{
		...
		$this->assertSame(0, $config->httpbl_threat, 'httpbl_threat must be clamped to 0 min');
	}
	...
	public function test_httpbl_maxage_floored_at_zero(): void
	{
		...
		$this->assertGreaterThanOrEqual(0, $config->httpbl_maxage, 'httpbl_maxage must not be negative');
	}
	...
	public function test_dns_verification_timeout_clamped_min(): void
	{
		...
		$this->assertGreaterThanOrEqual(50, $config->dns_verification_timeout_ms, 'dns_verification_timeout_ms must be clamped to 50ms min');
	}
	...
	public function test_dns_verification_timeout_clamped_max(): void
	{
		...
		$this->assertLessThanOrEqual(2000, $config->dns_verification_timeout_ms, 'dns_verification_timeout_ms must be clamped to 2000ms max');
	}
	...
	public function test_dns_verification_positive_ttl_floored(): void
	{
		...
		$this->assertGreaterThanOrEqual(3600, $config->dns_verification_positive_ttl, 'dns_verification_positive_ttl must be >= 3600 (1 hour min)');
	}
	...
	public function test_dns_verification_negative_ttl_floored(): void
	{
		...
		$this->assertGreaterThanOrEqual(60, $config->dns_verification_negative_ttl, 'dns_verification_negative_ttl must be >= 60 (1 minute min)');
	}
	...
	public function test_dynamic_ip_ranges_ttl_floored(): void
	{
		...
		$this->assertGreaterThanOrEqual(3600, $config->dynamic_ip_ranges_ttl, 'dynamic_ip_ranges_ttl must be >= 3600');
	}
	...
	public function test_rate_limit_global_requests_floored(): void
	{
		...
		$this->assertGreaterThanOrEqual(1, $config->rate_limits['global']['requests'], 'rate_limits.global.requests must be >= 1');
	}
	...
	public function test_rate_limit_window_floored(): void
	{
		...
		$this->assertGreaterThanOrEqual(1, $config->rate_limits['global']['window'], 'rate_limits.global.window must be >= 1');
	}
