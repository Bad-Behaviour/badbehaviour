...
class BadBehaviourTest extends TestCase
{
    ...
    /**
     * Static skip is the FIRST check in run(). It must match common
    ...
    public function test_should_skip_static_matches_js(): void
    {
        ...
        $this->assertTrue(
            $method->invoke($bb, '/path/to/file.js'),
            ".js files must be skipped"
        );
    }
    ...
    public function test_should_skip_static_matches_query_strings(): void
    {
        ...
        $this->assertTrue(
            $method->invoke($bb, '/style.css?v=12345'),
            ".css with query string must be skipped"
        );
        $this->assertTrue(
            $method->invoke($bb, '/app.js?version=1.2.3'),
            ".js with query string must be skipped"
        );
    }
    ...
    public function test_should_skip_static_matches_path_prefix(): void
    {
        ...
        $this->assertTrue(
            $method->invoke($bb, '/static/images/logo.png'),
            "/static/ path prefix must be skipped"
        );
        $this->assertTrue(
            $method->invoke($bb, '/assets/css/style.css'),
            "/assets/ path prefix must be skipped"
        );
    }
    ...
    public function test_should_skip_static_does_not_match_html(): void
    {
        ...
        $this->assertFalse(
            $method->invoke($bb, '/page.html'),
            ".html must NOT be skipped (needs detection)"
        );
    }
    ...
    public function test_should_skip_static_does_not_match_php(): void
    {
        ...
        $this->assertFalse(
            $method->invoke($bb, '/index.php'),
            ".php must NOT be skipped"
        );
    }
    ...
    public function test_should_skip_static_with_empty_config(): void
    {
        ...
        $this->assertTrue(
            $method->invoke($bb, '/style.css'),
            "static skip must work even with empty config (defaults must apply)"
        );
    }
     ...
     * BEFORE any detection or logging. This is what prevents the
     * SQLite lock storm.
     */
    public function test_run_skips_static_assets_without_logging(): void
    {
        ...
        $this->assertTrue(
            $result->is_allowed(),
            "static asset must return ALLOWED without detection"
        );
        $this->assertFalse(
            $result->is_actionable(),
            "static asset must not be actionable"
        );
        $this->assertSame(
            ResultCode::ALLOWED,
            $result->code,
            "static asset code must be ALLOWED"
        );
    }
    ...
    public function test_log_and_return_skips_allowed_when_verbose_false(): void
    {
        ...
        $this->assertFalse(
            $this->config->verbose,
            'precondition: verbose must be false for this test'
        );
     ...
     * monitor-only mode. These are tested via run_test_package().
     */
    public function test_maybe_demote_empty_ua_stays_enforced_in_monitor_only(): void
    {
        ...
        $this->assertSame(
            ...
            $result->code,
            'empty UA must produce BLOCKED_MALICIOUS_UA'
        );
        $this->assertSame(
            EnforcementAction::ENFORCED,
            $result->enforcement,
            'empty UA must STAY enforced in monitor-only mode'
        );
        $this->assertTrue(
            $result->is_actionable(),
            'empty UA must be actionable'
        );
    }
    ...
    public function test_maybe_demote_raw_xss_stays_enforced_in_monitor_only(): void
    {
        ...
        if ($result->code->is_blocked()) {
            $this->assertSame(
                EnforcementAction::ENFORCED,
                $result->enforcement,
                'raw_uri attack must STAY enforced in monitor-only mode'
            );
            $this->assertSame(
                'raw_uri',
                $result->metadata['tier'] ?? null,
                'metadata[tier] must be raw_uri (the demotion exception marker)'
            );
        }
    ...
    public function test_maybe_demote_demotes_normal_block_in_monitor_only(): void
    {
        ...
        $this->assertTrue(
            $result->is_monitored(),
            'normal block in monitor-only must be demoted to MONITORED'
        );
        $this->assertFalse(
            $result->is_actionable(),
            'MONITORED result must NOT be actionable'
        );
        $this->assertTrue(
            $result->reaches_application(),
            'MONITORED result must reach the application'
        );
    }
    ...
    public function test_maybe_demote_does_not_demote_in_normal_strictness(): void
    {
        ...
        $this->assertFalse(
            $result->is_monitored(),
            'normal strictness must NOT demote (result stays enforced if matched)'
        );
        $this->assertSame(
            EnforcementAction::ENFORCED,
            $result->enforcement,
            'normal strictness must keep enforcement'
        );
    }
    ...
    public function test_maybe_demote_passes_through_allowed(): void
    {
        ...
        $this->assertSame(
            ResultCode::ALLOWED,
            $result->code,
            'legitimate browser must be ALLOWED'
        );
        $this->assertSame(
            EnforcementAction::ALLOWED,
            $result->enforcement,
            'enforcement must be ALLOWED for ALLOWED result'
        );
    }
    ...
    public function test_is_whitelisted_with_empty_whitelist(): void
    {
        ...
        $this->assertFalse(
            $method->invoke($bb, $pkg),
            "no whitelist → no IP matches"
        );
    }
    ...
    public function test_handle_result_on_allowed_does_not_throw(): void
    {
        ...
        try {
            ...
            $this->fail(
                'handle_result() must not throw on ALLOWED result. '
    ...
    public function test_handle_result_on_monitored_does_not_throw(): void
    {
        ...
        $monitored = Result::monitored_from(Result::block(
            ResultCode::BLOCKED_MALICIOUS_UA,
            'Test monitored',
            $pkg
        ));
        ...
        try {
            ...
            $this->fail(
                'handle_result() must not throw on MONITORED result. '
                . 'Got: ' . get_class($e) . ': ' . $e->getMessage()
            );
        } finally {
            ob_end_clean();
    ...
    {
    	$safe_config = Configuration::from_array(
    		\BadBehaviour\Util\SafeMode::settings('test_log'),
    		$this->adapter
    		);
    	...
    	$this->assertTrue(
    		$diag['monitor_only_effective'],
    		'safe-mode settings must produce monitor_only_effective=true'
    		);
    ...
    public function test_diagnostics_reflects_monitor_only_strictness(): void
    {
        ...
        $this->assertTrue(
            $diag['monitor_only'],
            'monitor-only strictness must set monitor_only=true'
        );
    }
    ...
    public function test_is_in_safe_mode_false_with_valid_config(): void
    {
        ...
        $this->assertFalse(
            $bb->is_in_safe_mode(),
            'valid config must not trigger safe-mode'
        );
    }
    ...
    public function test_is_monitor_only_effective_true_with_monitor_only(): void
    {
        ...
        $this->assertTrue(
            $bb->is_monitor_only_effective(),
            'monitor-only strictness must set is_monitor_only_effective=true'
        );
    }
    ...
    public function test_is_monitor_only_effective_true_when_all_defenses_off(): void
    {
    	...
    	$this->assertFalse(
    		...
    		'dns_verification must be off'
    		);
    	$this->assertFalse(
    		$config->rate_limit_enabled,
    		'rate_limit must be off'
    		);
    	$this->assertFalse(
    		$config->enable_behavioral_analysis,
    		'behavioral must be off'
    		);
    	$this->assertFalse(
    		$config->enable_fingerprinting,
    		'fingerprinting must be off'
    		);
    	$this->assertFalse(
    	...
    	$this->assertTrue(
    		$bb->is_monitor_only_effective(),
    		'all defenses off must trigger is_monitor_only_effective=true'
    		);
    }
    ...
    public function test_with_registry_rebuilds_bot_detector(): void
    {
        ...
        $this->assertSame(
            $new_registry,
            $clone->get_registry(),
            'get_registry must return the new registry'
        );
    }
    ...
    public function test_with_adapter_loads_settings_from_adapter(): void
    {
        ...
        $this->assertSame(
            $this->adapter,
            $bb->get_registry() !== null ? $this->adapter : $this->adapter,
            'adapter must be wired through'
        );
    }
    ...
    public function test_with_adapter_with_overrides(): void
    {
        ...
        $this->assertSame(
            'full',
            $bb->diagnostics()['preset'],
            'overrides must take precedence'
        );
    }
    ...
    public function test_install_once_runs_only_once(): void
    {
    	...
    	if (!$prop->getValue($bb)) {
    		$this->markTestSkipped(
    			'install_done not set — likely static skip matched; '
    			. 'behavior verified by other tests'
    			);
    		return;
    	}
    ...
    public function test_create_challenge_builtin_by_default(): void
    {
        ...
        $this->assertInstanceOf(
            \BadBehaviour\Challenge\BuiltinChallenge::class,
            $challenge
        );
    }
    ...
    public function test_create_challenge_hcaptcha(): void
    {
        ...
        $this->assertInstanceOf(
            \BadBehaviour\Challenge\HCaptchaChallenge::class,
            $challenge
        );
    }
    ...
    public function test_create_challenge_recaptcha(): void
    {
        ...
        $this->assertInstanceOf(
            \BadBehaviour\Challenge\RecaptchaChallenge::class,
            $challenge
        );
    }
    ...
    public function test_create_challenge_turnstile(): void
    {
        ...
        $this->assertInstanceOf(
            \BadBehaviour\Challenge\TurnstileChallenge::class,
            $challenge
        );
    }
    ...
    {
        $config = Configuration::from_array([
            'challenge' => ['provider' => 'unknown-provider'],
        ], $this->adapter);
        $bb = new BadBehaviour($config);
        ...
        $this->assertInstanceOf(
            \BadBehaviour\Challenge\BuiltinChallenge::class,
            $challenge,
            'unknown provider must fall back to BuiltinChallenge'
        );
    }
    ...
    public function test_check_custom_rules_ip_block(): void
    {
        ...
        $pkg = RequestPackage::create_for_test(
            user_agent: 'Mozilla/5.0',
            ip: '203.0.113.50',  // in blocked range
            uri: '/'
        );
