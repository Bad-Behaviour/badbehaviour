...
class AssetScrapingDetectorTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $config = Configuration::from_array([
            ...
            'asset_extensions' => ['png', 'jpg', 'pdf', 'docx'],
    ...
    {
        $profile = ['html_requests' => 0, 'asset_requests' => 0];

        $this->adapter->method('get_behavior_profile')
            ->willReturnCallback(function() use (&$profile) {
                return $profile;
            });

        $this->adapter->method('save_behavior_profile')
            ->willReturnCallback(function($sid, $newProfile, $ttl) use (&$profile) {
                $profile = $newProfile;
                return true;
            });
    ...
    public function test_session_with_html_loads_allowed(): void
    {
        ...
        $profile = ['html_requests' => 5, 'asset_requests' => 0];
    ...
    public function test_html_load_increments_counter(): void
    {
        ...
        $package1 = RequestPackage::create_for_test(
            'Mozilla/5.0 Chrome/120',
            '203.0.113.1',
            'GET',
            '/index.html',
        ...
        for ($i = 0; $i < 25; $i++) {
            $package2 = RequestPackage::create_for_test(
                'Mozilla/5.0 Chrome/120',
                '203.0.113.1',
                'GET',
                '/img' . $i . '.png',
    ...
    public function test_asset_pattern_per_ip_isolation(): void
    {
        ...
        for ($i = 0; $i < 60; $i++) {
            $package_a = RequestPackage::create_for_test(
                'Mozilla/5.0 Chrome/120',
                '198.51.100.70',
                'GET',
                '/img' . $i . '.png',
        ...
        for ($i = 0; $i < 60; $i++) {
            $package_b = RequestPackage::create_for_test(
                'Mozilla/5.0 Chrome/120',
                '198.51.100.71',
                'GET',
                '/img' . $i . '.png',
    ...
    public function test_unknown_extension_returns_null(): void
    {
        ...
        $this->adapter->expects($this->never())->method('increment_counter');
        $this->adapter->expects($this->never())->method('get_behavior_profile');
