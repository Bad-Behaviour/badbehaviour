...
final class BotDetectorCategoryOverrideTest extends TestCase
{
	...
	private function make_detector(?Configuration $config = null, ?InMemoryRegistry $registry = null): BotDetector
	{
	...
	private function make_definition(BotCategory $category, BotAction $default_action = BotAction::ALLOW, string $id = 'test_bot', ?string $robots_txt_token = null, array $user_agent_patterns = [
		'TestBot/1.0'
	], array $ip_ranges = [], bool $verify_dns = false, array $dns_suffixes = []): BotDefinition
	{
		return new BotDefinition(id: $id, name: 'Test Bot', user_agent_patterns: $user_agent_patterns, host_patterns: [], ip_ranges: $ip_ranges, verify_dns: $verify_dns, dns_suffixes: $dns_suffixes, category: $category, robots_txt_token: $robots_txt_token, default_action: $default_action);
	}

	private function run_one(BotDetector $detector, BotDefinition $def, bool $verified = false): ?Result
	{
		$ua = $def->user_agent_patterns[0] . ' (Mozilla compatible)';
		$package = RequestPackage::create_for_test(user_agent: $ua, ip: '203.0.113.10');
	...
	public function test_cloud_infrastructure_allowed_even_when_in_blocked_list(): void
	{
		...
		$registry = new InMemoryRegistry([
			'cloudflare_health' => $this->make_definition(BotCategory::CLOUD_INFRASTRUCTURE, default_action: BotAction::ALLOW, id: 'cloudflare_health', user_agent_patterns: [
		...
		$this->assertSame(ResultCode::ALLOWED, $result->code, 'CLOUD_INFRASTRUCTURE must produce code=ALLOWED regardless of category overrides');
	...
	{
		$registry = new InMemoryRegistry([
			'aws_elb_health' => $this->make_definition(BotCategory::CLOUD_INFRASTRUCTURE, default_action: BotAction::BLOCK, id: 'aws_elb_health', user_agent_patterns: [
				'ELB-HealthChecker'
			])
		]);
		...
		$this->assertSame(ResultCode::ALLOWED, $result->code, 'Even with default_action=BLOCK, CLOUD_INFRASTRUCTURE is forced to ALLOW');
	}
	...
	public function test_cloud_infrastructure_default_action_runtime_contract(): void
	{
		...
		foreach ($default->cloud_infrastructure() as $def) {
			$this->assertSame(BotAction::ALLOW, $def->default_action, "DefaultRegistry ships {$def->id} with default_action={$def->default_action->value}, expected ALLOW");
		}
	}
	...
	public function test_category_override_wins_over_default_logic(string $list_key, ResultCode $expected_code, ?EnforcementAction $expected_enforcement): void
	{
		$config = $this->make_config([
			'bot_categories' => [
				$list_key => [
		...
		$registry = new InMemoryRegistry([
			'googlebot' => $this->make_definition(BotCategory::SEARCH_ENGINE, default_action: BotAction::ALLOW, id: 'googlebot', user_agent_patterns: [
				'Googlebot'
			], verify_dns: true, dns_suffixes: [
				'googlebot.com'
			])
		]);
		...
		$this->assertInstanceOf(Result::class, $result);
		$this->assertSame($expected_code, $result->code, "category override list '$list_key' must yield code {$expected_code->value}");
	...
	public function test_override_priority_blocked_beats_allowed(): void
	{
		...
		$registry = new InMemoryRegistry([
			'bingbot' => $this->make_definition(BotCategory::SEARCH_ENGINE, default_action: BotAction::ALLOW, id: 'bingbot', user_agent_patterns: [
				'bingbot'
			])
		]);
		$detector = $this->make_detector($config, $registry);
	...
	{
		$registry = new InMemoryRegistry([
			$bot_id => $this->make_definition($category, default_action: BotAction::CHALLENGE, id: $bot_id, user_agent_patterns: [
				$bot_id
			])
		]);
		$detector = $this->make_detector(null, $registry);
		...
		$this->assertSame(ResultCode::ALLOWED, $result->code, "{$category->value} / verified → ALLOWED expected");
	}
	...
	public function test_ai_crawler_allowed_when_token_in_allowed_list(): void
	{
		...
		$registry = new InMemoryRegistry([
			'gptbot' => $this->make_definition(BotCategory::AI_CRAWLER, default_action: BotAction::CHALLENGE, id: 'gptbot', robots_txt_token: 'GPTBot', user_agent_patterns: [
				'GPTBot'
			])
		]);
		$detector = $this->make_detector($config, $registry);
		...
		$this->assertSame(ResultCode::ALLOWED, $result->code, 'AI crawler listed in allowed[] → ALLOW even if strict + block_unverified');
	}
	...
	public function test_ai_crawler_blocked_when_strict_and_unverified(): void
	{
		...
		$this->assertSame(ResultCode::BLOCKED_AI_CRAWLER, $result->code, 'strict=true + unverified → BLOCK');
	}
	...
	public function test_ai_crawler_challenged_when_not_strict_and_unverified(): void
	{
		...
		$registry = new InMemoryRegistry([
			'claude' => $this->make_definition(BotCategory::AI_CRAWLER, default_action: BotAction::BLOCK, id: 'claude', robots_txt_token: 'ClaudeBot', user_agent_patterns: [
				'ClaudeBot'
			])
		]);
		$detector = $this->make_detector($config, $registry);
		...
		$this->assertSame(ResultCode::CHALLENGE_REQUIRED, $result->code, 'strict=false + unverified → CHALLENGE (default for AI)');
	}
	...
	public function test_ai_crawler_blocked_when_block_unverified_and_unverified(): void
	{
		...
		$registry = new InMemoryRegistry([
			'mistral' => $this->make_definition(BotCategory::AI_CRAWLER, default_action: BotAction::ALLOW, id: 'mistral', robots_txt_token: 'MistralBot', user_agent_patterns: [
				'MistralBot'
			])
		]);
		$detector = $this->make_detector($config, $registry);
		...
		$this->assertSame(ResultCode::BLOCKED_AI_CRAWLER, $result->code, 'block_unverified=true + unverified → BLOCK');
	}
	...
	public function test_seo_crawler_uses_default_action_when_verified(): void
	{
		...
		$registry = new InMemoryRegistry([
			'semrush' => $this->make_definition(BotCategory::SEO_CRAWLER, default_action: BotAction::CHALLENGE, id: 'semrush', user_agent_patterns: [
				'SemrushBot'
			], ip_ranges: [
				'203.0.113.0/24'
			] // covers test IP
		...
		$this->assertSame(ResultCode::CHALLENGE_REQUIRED, $result->code, 'SEO crawler verified → CHALLENGE (matches default_action)');
	}

	public function test_seo_crawler_blocked_when_unverified(): void
	{
		$registry = new InMemoryRegistry([
			'ahrefs' => $this->make_definition(BotCategory::SEO_CRAWLER, default_action: BotAction::ALLOW, id: 'ahrefs', user_agent_patterns: [
				'AhrefsBot'
			])
		]);
		$detector = $this->make_detector(null, $registry);
		...
		$this->assertSame(ResultCode::BLOCKED_SEO_CRAWLER, $result->code, 'SEO crawler unverified → BLOCK regardless of default_action');
	}
	...
	public function test_search_engine_blocked_when_unverified(): void
	{
		...
		$this->assertSame(ResultCode::BLOCKED_BOT, $result->code, 'Unverified search engine → BLOCK when strict_search_engines enabled');
	}
	...
	public function test_search_engine_allowed_when_verified(): void
	{
		...
		$registry = new InMemoryRegistry([
			'googlebot' => $this->make_definition(BotCategory::SEARCH_ENGINE, default_action: BotAction::BLOCK, // contradictory default
			id: 'googlebot', user_agent_patterns: [
				'Googlebot'
			], ip_ranges: [
				'203.0.113.0/24'
			] // covers test IP
		...
		$this->assertSame(ResultCode::ALLOWED, $result->code, 'Verified search engine → ALLOWED');
	}
	...
	public function test_social_crawler_allowed_when_verified(): void
	{
		...
		$registry = new InMemoryRegistry([
			'facebook' => $this->make_definition(BotCategory::SOCIAL_CRAWLER, default_action: BotAction::BLOCK, id: 'facebook', user_agent_patterns: [
				'facebookexternalhit'
			], ip_ranges: [
				'203.0.113.0/24'
			])
		...
		$this->assertSame(ResultCode::ALLOWED, $result->code, 'Verified social crawler → ALLOWED (link previews must work)');
	}
	...
	public function test_social_crawler_logged_only_when_unverified(): void
	{
		...
		$registry = new InMemoryRegistry([
			'twitter' => $this->make_definition(BotCategory::SOCIAL_CRAWLER, default_action: BotAction::CHALLENGE, id: 'twitter', user_agent_patterns: [
				'Twitterbot'
			])
		]);
		$detector = $this->make_detector(null, $registry);
		...
		$this->assertSame(ResultCode::ALLOWED, $result->code, 'Unverified social crawler → LOG_ONLY; production maps LOG_ONLY to code=ALLOWED');
	}
	...
	{
		$registry = new InMemoryRegistry([
			'shodan' => $this->make_definition(BotCategory::SECURITY_SCANNER, default_action: BotAction::BLOCK, id: 'shodan', user_agent_patterns: [
				'Shodan'
			])
		]);
		$detector = $this->make_detector(null, $registry);
	...
	public function test_residential_proxy_blocked_via_user_override(): void
	{
		...
		$registry = new InMemoryRegistry([
			'brightdata' => $this->make_definition(BotCategory::RESIDENTIAL_PROXY, default_action: BotAction::ALLOW, // overridden
			id: 'brightdata', user_agent_patterns: [
				'BrightData'
			])
		]);
		$detector = $this->make_detector($config, $registry);
		...
		$this->assertSame(ResultCode::BLOCKED_BOT, $result_verified->code, 'Residential proxy with bot_categories.blocked → BLOCK (verified)');
		$this->assertSame(ResultCode::BLOCKED_BOT, $result_unverified->code, 'Residential proxy with bot_categories.blocked → BLOCK (unverified)');
	}
	...
	public function test_residential_proxy_default_action_when_no_override(): void
	{
		...
		$registry = new InMemoryRegistry([
			'brightdata_permissive' => $this->make_definition(BotCategory::RESIDENTIAL_PROXY, default_action: BotAction::ALLOW, id: 'brightdata_permissive', user_agent_patterns: [
				'BrightDataPermissive'
			])
		]);
		$detector = $this->make_detector(null, $registry);
		...
		$this->assertSame(ResultCode::ALLOWED, $result->code, 'Residential proxy without user override → falls through to default_action');
	}
	...
	public function test_residential_proxy_default_action_in_default_registry(): void
	{
		...
		$this->assertNotNull($brightdata, 'DefaultRegistry must ship the brightdata residential-proxy definition');
		$this->assertSame(BotAction::BLOCK, $brightdata->default_action, 'brightdata must ship with default_action=BLOCK (out-of-box residential proxy block)');
	}
	...
	public function test_residential_proxy_allowed_via_user_override(): void
	{
		...
		$registry = new InMemoryRegistry([
			'brightdata' => $this->make_definition(BotCategory::RESIDENTIAL_PROXY, default_action: BotAction::BLOCK, id: 'brightdata', user_agent_patterns: [
				'BrightData'
			])
		]);
		$detector = $this->make_detector($config, $registry);
		...
		$this->assertSame(ResultCode::ALLOWED, $result->code, 'residential_proxy is NOT a safety category — allowed[] must work');
	}
	...
	{
		$registry = new InMemoryRegistry([
			'unknown_bot' => $this->make_definition(BotCategory::UNKNOWN, default_action: BotAction::CHALLENGE, id: 'unknown_bot', user_agent_patterns: [
				'MysteryBot'
			])
		]);
		$detector = $this->make_detector(null, $registry);
		...
		$this->assertSame(ResultCode::CHALLENGE_REQUIRED, $result->code, 'Unknown category → fall through to definition->default_action (CHALLENGE here)');
	}

	public function test_malicious_category_falls_back_to_definition_default_action(): void
	{
		$registry = new InMemoryRegistry([
			'malware_xyz' => $this->make_definition(BotCategory::MALICIOUS, default_action: BotAction::BLOCK, id: 'malware_xyz', user_agent_patterns: [
				'MalwareXYZ/1.0'
			])
		]);
		$detector = $this->make_detector(null, $registry);
	...
	{
		$registry = new InMemoryRegistry([
			'brightdata' => $this->make_definition(BotCategory::RESIDENTIAL_PROXY, default_action: BotAction::BLOCK, // makes BLOCK the fallthrough
			id: 'brightdata', user_agent_patterns: [
				'BrightData'
			])
		]);
