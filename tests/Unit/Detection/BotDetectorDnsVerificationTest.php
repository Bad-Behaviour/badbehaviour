...
class BotDetectorDnsVerificationTest extends TestCase
{
	...
	protected function setUp(): void
	{
		...
		$this->registry = new InMemoryRegistry([
			'testbot' => new BotDefinition(id: 'testbot', name: 'Test Bot', user_agent_patterns: [
				'TestBot/1.0'
			], host_patterns: [], ip_ranges: [], verify_dns: true, dns_suffixes: [
				'testbot.example.com'
			], category: BotCategory::SEARCH_ENGINE, robots_txt_token: 'TestBot', default_action: BotAction::ALLOW)
		]);

		$this->detector = new BotDetector($this->config, $this->adapter, $this->registry);

		$this->detector->set_dns_resolvers(reverse: function (string $ip) {
	...
	{
		return RequestPackage::create_for_test(user_agent: $ua, ip: $ip, method: 'GET', uri: '/test');
	}
	...
	public function test_cold_cache_legit_bot_verified_and_allowed(): void
	{
		...
		$this->assertSame(604800, $ttl, 'Positive TTL must match dns_verification_positive_ttl (7 days)');
	}
	...
	public function test_cold_cache_spoofed_bot_blocked_and_cached_negative(): void
	{
		...
		$ttl = $cached['expires'] - $cached['fetched'];
		$this->assertSame(86400, $ttl, 'Negative TTL must match dns_verification_negative_ttl (1 day)');
	}
	...
	public function test_warm_cache_avoids_dns_call(): void
	{
		...
		$this->assertSame(0, $this->reverse_call_count, 'Warm cache must skip the DNS resolver entirely');
	}
	...
	public function test_expired_cache_triggers_re_verification(): void
	{
		...
		$this->assertTrue($result->is_allowed(), 'After expiry, re-verification must run and the bot must pass now');
		$this->assertSame(1, $this->reverse_call_count, 'Expired cache must trigger a fresh DNS lookup');
	}
	...
	public function test_ipv6_bot_uses_binary_cache_key(): void
	{
		...
		$this->assertArrayHasKey($expected_key, $this->adapter->cache, 'IPv6 cache key must use binary (inet_pton) form');
	}
	...
	public function test_normalize_ipv6_canonical_form(): void
	{
		...
		$this->assertSame($bin_a, $bin_b, 'IPv6 canonical form must normalize so cache keys match');
	}
	...
	public function test_kill_switch_skips_dns_entirely(): void
	{
		...
		$detector->set_dns_resolvers(reverse: function () {
			$this->reverse_call_count ++;
			return false;
		}, forward: function () {
			$this->forward_call_count ++;
		...
		$this->assertSame(0, $this->reverse_call_count, 'Kill switch must skip DNS even if resolver is set');
		// Search engine without IP match + no DNS verification → BLOCKED
		$this->assertTrue($result->is_blocked(), 'Without DNS verification, an unverified search engine claim is blocked');
	}
	...
	public function test_strict_mode_accepts_when_forward_dns_confirms(): void
	{
		...
		$detector->set_dns_resolvers(reverse: fn (string $ip) => $ip === '1.2.3.4' ? 'crawl-1-2-3.testbot.example.com' : false, forward: fn (string $host, int $type) => [
			[
				'ip' => '1.2.3.4'
			]
		]);
		...
		$this->assertTrue($result->is_allowed(), 'Strict mode with matching A record must accept');
	}
	...
	public function test_strict_mode_rejects_when_forward_dns_fails(): void
	{
		...
		$detector = new BotDetector($strict_config, $adapter, $this->registry);
		$detector->set_dns_resolvers(reverse: fn (string $ip) => $ip === '1.2.3.4' ? 'crawl.testbot.example.com' : false, forward: fn () => [] // empty A record list
		);

		$result = $detector->detect($this->make_package('1.2.3.4', 'TestBot/1.0'));

		$this->assertTrue($result->is_blocked(), 'Strict mode with empty A record must reject (PTR spoof protection)');
	}
	...
	public function test_strict_mode_rejects_ipv6_without_aaaa_match(): void
	{
		...
		$detector = new BotDetector($strict_config, $adapter, $this->registry);
		$detector->set_dns_resolvers(reverse: fn (string $ip) => $ip === '2a03:2880::1' ? 'crawl.testbot.example.com' : false, 
			// Empty AAAA record → strict mode rejects
			forward: fn () => []);

		$result = $detector->detect($this->make_package('2a03:2880::1', 'TestBot/1.0'));

		$this->assertTrue($result->is_blocked(), 'Strict mode must reject IPv6 without matching AAAA record');
	}
	...
	public function test_resolver_hook_can_be_swapped_via_setter(): void
	{
		...
		$this->assertTrue($result2->is_blocked(), 'After swapping to a malicious resolver, IPs must be blocked');
	...
	public function test_set_dns_resolvers_invalidates_per_request_cache(): void
	{
		...
		$detector->set_dns_resolvers(reverse: fn () => 'cached.testbot.example.com', forward: fn () => []);

		// Swap resolvers — per-request cache must be invalidated.
		$detector->set_dns_resolvers(reverse: fn () => 'evil.com', forward: fn () => []);
	...
	/**
	 * Bot defines multiple DNS suffixes.
	 * PTR resolves to a host matching
	...
	public function test_multi_suffix_first_match_succeeds(): void
	{
		$registry = new InMemoryRegistry([
			'multibot' => new BotDefinition(id: 'multibot', name: 'Multi Suffix Bot', user_agent_patterns: [
				'MultiBot/1.0'
			], host_patterns: [], ip_ranges: [], verify_dns: true, dns_suffixes: [
				'facebook.com',
				'fbcdn.net',
				...
				category: BotCategory::SEARCH_ENGINE, robots_txt_token: 'MultiBot', default_action: BotAction::ALLOW)
		]);
		...
		$reverse_calls = [];
		$detector->set_dns_resolvers(reverse: function (string $ip) use (&$reverse_calls) {
			$reverse_calls[] = $ip;
			return match ($ip) {
		...
		$result = $detector->detect(RequestPackage::create_for_test(user_agent: 'MultiBot/1.0', ip: '1.2.3.4', method: 'GET', uri: '/test'));

		$this->assertTrue($result->is_allowed(), 'IP matching first suffix must be allowed');
		$this->assertSame([
			'1.2.3.4'
		], $reverse_calls, 'Only one reverse lookup needed when first suffix matches');
	}
	...
	{
		$registry = new InMemoryRegistry([
			'metabot' => new BotDefinition(id: 'metabot', name: 'Meta AI', user_agent_patterns: [
				'MetaBot/1.0'
			], host_patterns: [], ip_ranges: [], verify_dns: true, dns_suffixes: [
				'facebook.com',
				'fbcdn.net',
				'amazonaws.com' // ← PTR host will match this
			], category: BotCategory::SEARCH_ENGINE, robots_txt_token: 'MetaBot', default_action: BotAction::ALLOW)
		]);
		...
		$result = $detector->detect(RequestPackage::create_for_test(user_agent: 'MetaBot/1.0', ip: '1.2.3.4', method: 'GET', uri: '/test'));

		$this->assertTrue($result->is_allowed(), 'IP matching later suffix must be allowed');
		$this->assertCount(1, $reverse_calls, 'Per-IP PTR caching: only ONE reverse lookup needed regardless of suffix count');
	}

	/**
	 * Bot defines multiple DNS suffixes.
	 * PTR resolves to a host
	...
	public function test_multi_suffix_no_match_blocks(): void
	{
		...
		$detector->set_dns_resolvers(reverse: function (string $ip) {
		...
		$this->assertTrue($result->is_blocked(), 'IP matching no suffix must be blocked');
		...
		foreach ([
			...
			$this->assertArrayHasKey($key, $adapter->cache, "Negative cache entry for suffix '{$suffix}' must exist");
			$this->assertFalse($adapter->cache[$key]['value'], "Cache entry for '{$suffix}' must be negative (false)");
		}
	}
	...
	public function test_negative_cache_on_one_suffix_does_not_block_other_suffixes(): void
	{
		...
		$this->assertTrue($result->is_allowed(), 'Negative cache on facebook.com must NOT prevent verification on amazonaws.com');
	}
	...
	{
		$registry = new InMemoryRegistry([
			'unconfigured' => new BotDefinition(id: 'unconfigured', name: 'Misconfigured Bot', user_agent_patterns: [
				'Unconfigured/1.0'
			], host_patterns: [], ip_ranges: [], verify_dns: true, dns_suffixes: [], // ← empty array
			category: BotCategory::SEARCH_ENGINE, robots_txt_token: 'Unconfigured', default_action: BotAction::ALLOW)
		]);
		...
		$result = $detector->detect(RequestPackage::create_for_test(user_agent: 'Unconfigured/1.0', ip: '1.2.3.4', method: 'GET', uri: '/test'));

		$this->assertCount(0, $reverse_calls, 'Empty dns_suffixes must NOT trigger any reverse DNS call');
		$this->assertTrue($result->is_blocked(), 'Search engine without IP match + empty suffixes must be blocked ' . '(regression guard against off-by-one in the loop)');
	}
	 ...
	 * suffix iteration).
	 * The shared PTR cache means we don't even
	...
	public function test_multi_suffix_warm_cache_short_circuits(): void
	{
		...
		$this->assertCount(0, $reverse_calls, 'Warm caches (both PTR and per-suffix) must short-circuit all DNS');
		$this->assertTrue($result->is_allowed());
	}
}
