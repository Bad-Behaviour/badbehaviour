...
class BotDetectorTest extends TestCase
{
	...
	{
		return RequestPackage::create_for_test(user_agent: $ua, ip: $ip);
	}
	...
	public function test_known_search_engine_allowed(): void
	{
		...
		$package = $this->createPackage('Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)', '66.249.64.1');

		$result = $this->detector->detect($package);

		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'Googlebot from a static Google IP must be allowed without DNS');
	}
	 ...
	 * falls through to $def->default_action.
	...
	public function test_search_engine_unverified_with_strict_off_returns_default_action(): void
	{
		...
		$package = $this->createPackage('Mozilla/5.0 (compatible; DuckDuckBot/1.1; +http://duckduckgo.com/duckduckbot.html)', '192.0.2.1');

		$result = $this->detector->detect($package);

		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'Under strict_search_engines=false (default), unverified UA-only ' . 'search engines fall through to def->default_action, not BLOCK');
		$this->assertSame('search_engine', $result->metadata['bot_category'] ?? null, 'Bot is still correctly categorized as SEARCH_ENGINE even when allowed');
	}

	/**
	 * Strict search engines gate: with the flag ON, an unverified SE is
	 * hard-blocked.
	 * This is the explicit opt-in for operators seeing
	...
	public function test_search_engine_unverified_with_strict_on_returns_block(): void
	{
		...
		$package = $this->createPackage('Mozilla/5.0 (compatible; DuckDuckBot/1.1)', '192.0.2.1');

		$result = $detector->detect($package);

		$this->assertNotNull($result);
		$this->assertTrue($result->is_enforced_block(), 'strict_search_engines=true must hard-block unverified SEs');
	 ...
	 * gate.
	 * Under strict_search_engines=true, a verified Googlebot must
	...
	public function test_search_engine_verified_short_circuits_strict_gate(): void
	{
		...
		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'Verified Googlebot must pass even when strict_search_engines=true');
	}
	...
	public function test_ai_crawler_allowed_when_configured(): void
	{
		...
		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'GPTBot from a static OpenAI IP must be allowed');
	}
	...
	public function test_social_crawler_unverified_log_only(): void
	{
		...
		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'Unverified social crawler → LOG_ONLY → ALLOWED code with ENFORCED enforcement');
	}
	...
	public function test_dynamic_ranges_cached(): void
	{
		...
		$package = $this->createPackage('Mozilla/5.0 (compatible; Googlebot/2.1)', '64.233.160.1');

		$result = $detector->detect($package);

		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'Googlebot IP from dynamic-range cache must be allowed');
	}
	...
	public function test_dynamic_ranges_disabled_uses_static(): void
	{
		...
		$package = $this->createPackage('Mozilla/5.0 (compatible; Googlebot/2.1)', '66.249.64.1');

		$result = $detector->detect($package);
		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'Without dynamic ranges, static googlebot range still applies');
	}
	...
	public function test_cloudflare_health_ip_always_allowed(): void
	{
		...
		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'Cloudflare probe IPs MUST be allowed — blocking them takes origin offline');
	}
	...
	public function test_aws_elb_health_ip_always_allowed(): void
	{
		...
		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'AWS ELB health check MUST be allowed (via CLOUD_INFRASTRUCTURE safety override)');
	}
	...
	public function test_gcp_load_balancer_health_always_allowed(): void
	{
		...
		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'GCP LB health probes MUST be allowed');
	}
	...
	public function test_azure_health_probe_always_allowed(): void
	{
		...
		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'Azure health probes MUST be allowed');
	}
	...
	public function test_fastly_health_always_allowed(): void
	{
		...
		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'Fastly health probes MUST be allowed');
	}
	...
	public function test_coccoc_vietnam_bot_allowed_under_lenient_default(): void
	{
		...
		$package = $this->createPackage('Mozilla/5.0 (compatible; coccocbot/2.0; +http://coccoc.com)', '192.0.2.1');

		$result = $this->detector->detect($package);

		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'coccoc has verify_dns=false and no static ranges; lenient ' . 'strict_search_engines default allows UA-only SEs through');
	...
	public function test_mailru_allowed_under_lenient_default(): void
	{
		...
		$package = $this->createPackage('Mozilla/5.0 (compatible; Mail.RU_Bot/2.0)', '192.0.2.1');

		$result = $this->detector->detect($package);

		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'Mail.ru unverified under lenient default must pass (verifies ' . 'strict_search_engines gate handles DNS-fail-true case)');
	}
	...
	public function test_coccoc_blocked_under_strict_mode(): void
	{
		...
		$this->assertTrue($result->is_enforced_block(), 'Under strict_search_engines=true, UA-only SEs are blocked');
		$this->assertSame(ResultCode::BLOCKED_BOT, $result->code);
	}
	...
	{
		$package = $this->createPackage('Mozilla/5.0 (compatible; Amazonbot/1.0; +https://developer.amazon.com/support/amazonbot)', '192.0.2.1');
	...
	{
		$package = $this->createPackage('Diffbot/2.0 (+https://www.diffbot.com)', '192.0.2.1');
	...
	{
		$package = $this->createPackage('Mozilla/5.0 (compatible; BrightData/1.0)', '192.0.2.1');

		$result = $this->detector->detect($package);

		$this->assertTrue($result->is_blocked(), 'BrightData residential proxy must default to BLOCK');
		$this->assertSame(ResultCode::BLOCKED_BOT, $result->code, 'Residential proxy uses the generic BLOCKED_BOT code (see BotDetector::code_for_category)');
	}
	...
	public function test_facebook_catalog_allowed(): void
	{
		...
		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'Facebook Catalog MUST be allowed (product feed revenue)');
	}
	...
	public function test_feedly_allowed(): void
	{
		...
		$package = $this->createPackage('Feedly/1.0 (+https://www.feedly.com/fetcher.html)', '54.144.1.1');

		$result = $this->detector->detect($package);

		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'Feedly MUST be allowed (RSS reader brings real users)');
	}
	...
	public function test_apple_news_allowed(): void
	{
		...
		$this->assertNotNull($result);
		$this->assertTrue($result->is_allowed(), 'Apple News MUST be allowed (publisher visibility)');
	}
}