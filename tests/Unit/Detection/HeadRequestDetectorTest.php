...
class HeadRequestDetectorTest extends TestCase
{
	...
	{
		$package = RequestPackage::create_for_test('Mozilla/5.0 Chrome/120', '203.0.113.1', 'GET', '/page');
	...
	{
		$package = RequestPackage::create_for_test('Mozilla/5.0 Chrome/120', '203.0.113.1', 'HEAD', '/sitemap.xml');
	...
	{
		$package = RequestPackage::create_for_test('MyApp/2.0', '203.0.113.1', 'HEAD', '/api/v1/status');
	...
	{
		$package = RequestPackage::create_for_test('MyApp/2.0', '203.0.113.1', 'HEAD', '/wp-json/wp/v2/posts');
	...
	public function test_head_flood_blocked_after_threshold(): void
	{
		...
		$this->adapter->method('get_behavior_profile')->willReturnCallback(function ($sid) use (&$counter) {
			return [
				'head_count' => $counter
			];
		});

		$this->adapter->method('save_behavior_profile')->willReturnCallback(function ($sid, $profile, $ttl) use (&$counter) {
		...
		$result = null;
		for ($i = 0; $i < 21; $i ++) {
			$package = RequestPackage::create_for_test('Mozilla/5.0 Chrome/120', '203.0.113.1', 'HEAD', '/page' . $i, [
				'Referer' => 'https://example.com/admin'
			], [], $session_id);
			$result = $this->detector->detect($package);
		}
	...
	public function test_head_probing_blocked_after_threshold(): void
	{
		...
		$result = null;
		for ($i = 0; $i < 51; $i ++) {
			$package = RequestPackage::create_for_test('Mozilla/5.0 Chrome/120', '198.51.100.42', 'HEAD', '/page' . $i, [
				'Referer' => 'https://example.com/admin'
			]);
			$result = $this->detector->detect($package);
		}
	...
	public function test_disabled_returns_null(): void
	{
		...
		$package = RequestPackage::create_for_test('Mozilla/5.0 Chrome/120', '203.0.113.1', 'HEAD', '/page');
