...
class RateLimitDetectorTest extends TestCase
{
	...
	{
		return new RequestPackage(ip: '192.0.2.1', headers: [], headers_mixed: [
			'User-Agent' => 'Chrome/120'
		], request_method: $method, request_uri: $uri, server_protocol: 'HTTP/1.1', request_entity: [], user_agent: 'Chrome/120');
	}
	...
	public function test_different_ips_independent(): void
	{
		...
		$package1 = $package1->with_modified([
			'ip' => '192.0.2.1'
		]);

		$package2 = $this->create_package();
		$package2 = $package2->with_modified([
			'ip' => '192.0.2.2'
		]);
