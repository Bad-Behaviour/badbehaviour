...
final class OnDemandRefresherTest extends TestCase
{
     ...
     * @param (callable(int, int): int)|null $rng
     */
    private function makeRefresher(array $options = [], ?callable $rng = null): OnDemandRefresher
    {
        return new OnDemandRefresher(
            cache: $this->cache,
            registry: $this->registry,
            cloud: $this->cloud,
            options: $options,
            clock: fn(): int => $this->current_time,
            rng: $rng,
        );
    }
    ...
    public function testGate1ProbabilitySkipsMostRequests(): void
    {
        ...
        $refresher = $this->makeRefresher(['probability_denominator' => 1000], $rng);
        ...
        for ($i = 0; $i < 100; $i++) {
            ...
            $this->assertFalse(
                ...
                "Iteration {$i} should not schedule when RNG never rolls 1"
            );
    ...
    public function testGate1ProbabilityPassesOnExactRoll(): void
    {
        ...
        $refresher = $this->makeRefresher(
            ['probability_denominator' => 1000, 'min_age_seconds' => 0],
            $rng
        );
        $decision = $refresher->maybe_refresh();
    ...
    public function testGate1ProbabilityDenominatorZeroIsRejectedAtConstruction(): void
    {
        ...
        $this->makeRefresher(['probability_denominator' => 0]);
    }
    ...
    public function testGate3StalenessSkipsWhenFresh(): void
    {
        ...
        $this->cache->set(OnDemandRefresher::CACHE_KEY_MERGED, [
            'data' => ['googlebot' => ['1.2.3.4/32']],
            'fetched' => $this->current_time - 3600,
        ], 86400);

        $rng = fn(int $min, int $max): int => 1;
        $refresher = $this->makeRefresher(
            ['probability_denominator' => 1000, 'min_age_seconds' => 21600],
            $rng
        );
    ...
    public function testGate4MutexClearedAfterRefresh(): void
    {
        ...
        $this->assertNull(
            $this->cache->get(OnDemandRefresher::CACHE_KEY_LOCK),
            'do_refresh() must clear the lock when it finishes'
        );
    ...
    public function testGate4MutexReportsMutexLostOnSetFailure(): void
    {
        ...
        $refresher = new OnDemandRefresher(
            ...
            registry: $this->registry,
            cloud: $this->cloud,
            options: ['min_age_seconds' => 0], // force schedule
            clock: fn(): int => $this->current_time,
            rng: fn(int $min, int $max): int => 1,
        );
    ...
    public function testDoRefreshMergesAllBotFeeds(): void
    {
        ...
        $this->registry->addFeed('google', new FakeIpFeed([
            'googlebot' => ['1.2.3.4/32', '5.6.7.8/32'],
        ]));
        $this->registry->addFeed('openai', new FakeIpFeed([
            'gptbot' => ['9.10.11.12/32'],
        ]));
        $this->cloud->setRanges('aws', ['13.14.15.16/32', '17.18.19.20/32']);
        ...
        $this->assertSame(['1.2.3.4/32', '5.6.7.8/32'], $payload['data']['googlebot']);
        $this->assertSame(['9.10.11.12/32'], $payload['data']['gptbot']);
        $this->assertSame(['13.14.15.16/32', '17.18.19.20/32'], $payload['data']['aws_elb_health']);
    }
    ...
    public function testDoRefreshDeduplicatesOverlappingRanges(): void
    {
        ...
        $this->registry->addFeed('feed-b', new FakeIpFeed([
            'googlebot' => ['1.2.3.4/32', '9.10.11.12/32'],
        ]));
        ...
        $payload = $this->cache->get(OnDemandRefresher::CACHE_KEY_MERGED);
        $this->assertSame(
            ['1.2.3.4/32', '5.6.7.8/32', '9.10.11.12/32'],
    ...
    public function testDoRefreshContinuesOnPerFeedFailure(): void
    {
        $this->registry->addFeed('working', new FakeIpFeed([
            'googlebot' => ['1.2.3.4/32'],
        ]));
        $this->registry->addFeed('broken', new FakeIpFeed(
            data: [],
            throws: new \RuntimeException('feed endpoint down')
        ));
        ...
        $payload = $this->cache->get(OnDemandRefresher::CACHE_KEY_MERGED);
        $this->assertSame(['1.2.3.4/32'], $payload['data']['googlebot']);
    }

    public function testDoRefreshSkipsCacheWriteOnTotalFailure(): void
    {
        $this->registry->addFeed('broken-1', new FakeIpFeed(
            throws: new \RuntimeException('fail 1')
        ));
        $this->registry->addFeed('broken-2', new FakeIpFeed(
            throws: new \RuntimeException('fail 2')
        ));
        ...
        $this->assertNull(
            ...
            'Cache must remain unwritten on total failure'
        );
    }
    ...
    public function testDoRefreshHonorsFeedTimeoutBudget(): void
    {
        ...
        $skipped = array_filter(
            $result->feed_status,
            fn($s) => ($s['status'] ?? null) === 'skipped'
        );
        $this->assertGreaterThan(
            0,
            count($skipped),
            'Slow feeds should be skipped when budget is exhausted'
        );
    }
    ...
    public function testDoRefreshRespectsBotIdFilter(): void
    {
        ...
        $this->cloud->setRanges('aws', ['9.10.11.12/32']);
        ...
        $refresher = $this->makeRefresher(
            [
                'probability_denominator' => 1000,
                'min_age_seconds'         => 0,
                'bot_ids'                 => ['googlebot'], // Only this bot
            ],
            $rng
        );
        ...
        $this->assertArrayNotHasKey(
            'gptbot',
            $payload['data'],
            'gptbot must be filtered out by bot_ids option'
        );
        $this->assertArrayNotHasKey(
            'aws_elb_health',
            $payload['data'],
            'aws_elb_health must be filtered out (not in bot_ids allow-list)'
        );
    }
    ...
    public function testDoRefreshRespectsCloudProviderFilter(): void
    {
        ...
        $this->cloud->setRanges('aws', ['9.10.11.12/32']);
        $this->cloud->setRanges('cloudflare', ['13.14.15.16/32']);
        ...
        $refresher = $this->makeRefresher(
            [
                'probability_denominator' => 1000,
                'min_age_seconds'         => 0,
                'cloud_providers'         => ['cloudflare'], // Only this provider
            ],
            $rng
        );
        ...
        $this->assertArrayNotHasKey(
            'aws_elb_health',
            $payload['data'],
            'aws must be filtered out by cloud_providers option'
        );
    }
    ...
    {
        $this->registry->addFeed('ok', new FakeIpFeed(['googlebot' => ['1.2.3.4/32']]));
        $this->registry->addFeed('err', new FakeIpFeed(
    ...
    public function testDoRefreshHandlesCloudProviderThrowing(): void
    {
        ...
        $cloud_statuses = array_filter(
            $result->feed_status,
            fn($_, $k) => str_starts_with($k, 'cloud:'),
    ...
    {
        $refresher = $this->makeRefresher(['min_age_seconds' => 60]);
        $opts = $refresher->get_options();
    ...
    public function testDoRefreshClearsLockEvenOnException(): void
    {
        ...
        $this->assertNull(
            $this->cache->get(OnDemandRefresher::CACHE_KEY_LOCK),
            'Lock must be cleared even when feeds throw'
        );
    }
    ...
    public function testDoRefreshClearsLockEvenOnCacheWriteFailure(): void
    {
        ...
        $cache = new SelectiveThrowingCache(
            throw_on_key: OnDemandRefresher::CACHE_KEY_MERGED
        );
        $refresher = new OnDemandRefresher(
            cache: $cache,
            registry: $this->registry,
            cloud: $this->cloud,
            options: ['probability_denominator' => 1000, 'min_age_seconds' => 0],
            clock: fn(): int => $this->current_time,
            rng: fn(int $min, int $max): int => 1,
        );
        ...
        $this->assertNull(
            $cache->get(OnDemandRefresher::CACHE_KEY_LOCK),
            'Lock must be cleared even when cache write fails'
        );
    }
    ...
    public function testWasRefreshInvokedFalseBeforeDoRefresh(): void
    {
        ...
        $refresher->maybe_refresh();
        $this->assertFalse(
            $refresher->was_refresh_invoked(),
            'was_refresh_invoked reflects do_refresh() invocation, not maybe_refresh()'
        );
    ...
    public function testRefreshResultCountsFeeds(): void
    {
        $result = new RefreshResult(
            ...
            feed_status: [
                'feed-a' => ['status' => 'ok'],
                'feed-b' => ['status' => 'ok'],
                'feed-c' => ['status' => 'error', 'error' => 'down'],
                'feed-d' => ['status' => 'skipped', 'reason' => 'budget'],
            ],
            started_at: 1000,
            finished_at: 1001,
        );
    ...
    public function testEndToEndProbabilityGateDistributesCorrectly(): void
    {
        ...
        $refresher = $this->makeRefresher(
            ['probability_denominator' => 10, 'min_age_seconds' => 21600],
            $rng
        );
...
final class FakeCloudProvider extends CloudIpRangeProvider
{
     ...
     * (then clear). Lets tests simulate transient cloud API failures.
     */
    ...
    /**
     * @param array<string, string[]> $ranges provider => CIDRs
     */
...
final class FakeIpFeed implements IpFeedInterface
{
    ...
    public function __construct(
        ...
        private float $sleep_seconds = 0.0,
    ) {}
