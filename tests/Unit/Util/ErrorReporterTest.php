...
final class ErrorReporterTest extends TestCase
{
    ...
    public function testErrorRoutesThroughAdapterWhenAvailable(): void
    {
        ...
        ErrorReporter::error($adapter, 'something broke', ['key' => 'value']);
        ...
        $this->assertSame(['key' => 'value'], $adapter->calls[0]['context']);
    }
    ...
    public function testErrorFallsBackToErrorLogWhenAdapterIsNull(): void
    {
        ...
        ErrorReporter::error(null, 'null-adapter path', ['a' => 1]);
    ...
    public function testErrorWithContextContainingNonUtf8IsHandled(): void
    {
        ...
        // This must NOT throw
        ErrorReporter::error($adapter, 'utf8 test', ['bad' => $bad_utf8]);
    ...
    public function testAdapterWithNonLogMethodObjectIsHandled(): void
    {
        ...
        $adapter = new class implements AdapterInterface {
            ...
            public function query(string $sql): bool { return false; }
            public function get(string $k): mixed { return null; }
            public function set(string $k, mixed $v, int $ttl): bool { return true; }
            public function delete(string $k): bool { return true; }
            public function increment_counter(string $k, int $w): int { return 0; }
            public function get_counter(string $k): int { return 0; }
            public function get_behavior_profile(string $s): ?array { return null; }
            public function save_behavior_profile(string $s, array $p, int $ttl): bool { return true; }
            public function add_to_set(string $k, string $v, int $ttl): bool { return true; }
    ...
    public function testExposesOnlyStaticApi(): void
    {
        ...
        $publicMethods = array_filter(
            ...
            fn($m) => $m->getDeclaringClass()->getName() === ErrorReporter::class
        );
        ...
        foreach ($publicMethods as $m) {
            $this->assertTrue(
                $m->isStatic(),
                "Method {$m->getName()} must be static"
            );
        }
    ...
    public function testPublicApiSurface(): void
    {
        ...
        $expected = ['error', 'warning', 'fatal', 'reset'];
