...
final class LogRetentionTest extends TestCase
{
    ...
    private function makeRetention(
        ...
        return new LogRetention(
            ...
            max_total_seconds: $max_total_seconds ?? 0.5,
            clock: fn() => $this->fixedNow,
            rng: fn(int $min, int $max) => $this->rngRollsOne ? 1 : $max,
        );
    }
    ...
    {
        $this->config = $this->makeConfig(['log_retention' => ['enabled' => false]]);
    ...
    public function gate2_returns_cooldown_when_no_cache_available(): void
    {
    	...
    	$noCacheAdapter = new class implements \BadBehaviour\Core\Interfaces\AdapterInterface {
    		...
    		public function get_table_schema(string $t): string { return ''; }
    		...
    		public function delete(string $k): bool { return true; }
    ...
    public function gate3_skips_when_last_run_was_recent(): void
    {
        ...
        $this->adapter->seedCache(
            LogRetention::CACHE_KEY_LAST_RUN,
            $this->fixedNow - 3600,
            86400,
        );
    ...
    public function gate3_treats_far_future_timestamp_as_cold_start(): void
    {
        ...
        $this->adapter->seedCache(
            LogRetention::CACHE_KEY_LAST_RUN,
            $this->fixedNow + 86400, // 1 day in the future
            86400,
        );
    ...
    public function gate4_skips_when_lock_acquisition_fails(): void
    {
        ...
        $retention = new LogRetention(
            ...
            config: $this->config,
            cache: new class implements \BadBehaviour\Core\Interfaces\CacheInterface {
                public function get(string $k): mixed { return null; }
                public function set(string $k, mixed $v, int $t): bool { return false; }
                public function delete(string $k): bool { return true; }
                public function increment_counter(string $k, int $w): int { return 1; }
                public function get_counter(string $k): int { return 0; }
                public function get_set(string $k): array { return []; }
    ...
    public function do_cleanup_emits_age_based_delete(): void
    {
        ...
        $this->adapter = new RetentionTestAdapter(
            logTable: 'bad_behaviour',
            probeNewest: $ten_days_ago,
            probeTotal: 500,
        );
        $this->config = $this->makeConfig();
    ...
    public function do_cleanup_sanitizes_log_table_name(): void
    {
        ...
        $this->adapter = new RetentionTestAdapter(
            logTable: 'bad_behaviour`; DROP TABLE users; --',
            probeNewest: $this->fixedNow - 86400,
            probeTotal: 100,
        );
        $this->config = $this->makeConfig();
        ...
        $this->assertMatchesRegularExpression(
        	'/^DELETE FROM `[a-zA-Z0-9_]+` WHERE `date` < \d+$/',
        	$sql,
        );
    }
    ...
    public function maybe_cleanup_converts_exceptions_to_skip_decision(): void
    {
        ...
        $explodingCache = new class implements \BadBehaviour\Core\Interfaces\CacheInterface {
            ...
            public function set(string $k, mixed $v, int $t): bool { return true; }
            public function delete(string $k): bool { return true; }
            public function increment_counter(string $k, int $w): int { return 1; }
            public function get_counter(string $k): int { return 0; }
            public function get_set(string $k): array { return []; }
    ...
    {
        $r = new RetentionResult(
            success: true,
            rows_deleted: 42,
            iterations: 3,
            elapsed_seconds: 0.123456,
            cutoff_computed: 1_700_000_000,
            log_table: 't',
            limit_by: 'age',
            error: null,
        );
    ...
    {
    	$adapter = new RetentionTestAdapter(
    		probeNewest: 1_700_000_000,
    		probeTotal: 42,
    		);
