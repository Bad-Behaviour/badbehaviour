...
final class SafeConfigLoaderTest extends TestCase
{
	...
	public function testLoadReturnsArrayOnSuccess(): void
	{
		...
		$this->assertSame([
			'logging' => true,
			'verbose' => false
		], $result, 'load() must return the array verbatim');
	}
	...
	public function testLoadDoesNotCrashWhenAdapterLoggerThrows(): void
	{
		...
		$result = SafeConfigLoader::load('/no/such/path/' . bin2hex(random_bytes(8)) . '.php', $adapter, 'crash-tag');
	...
	public function testLoadWithoutAdapterDoesNotCrashOnAnyFailure(): void
	{
		...
		$result = SafeConfigLoader::load('/no/such/path/' . bin2hex(random_bytes(8)) . '.php', null);
		$this->assertNull($result);
	}
	...
	public function testFindExistingReturnsFirstExistingPath(): void
	{
		...
		$this->assertSame($a, SafeConfigLoader::find_existing([
			$a,
			$b,
			$c
		]));
	...
	public function testFindExistingSkipsMissingFiles(): void
	{
		...
		$this->assertSame($existing, SafeConfigLoader::find_existing([
			$missing,
			$also_missing,
			$existing
		]));
	...
	public function testFindExistingOrderMatters(): void
	{
		...
		$this->assertSame($b, SafeConfigLoader::find_existing([
			$b,
			$a
		]));
	}
	...
	public function testFindExistingReturnsStringType(): void
	{
		...
		$result = SafeConfigLoader::find_existing([
			$existing
		]);
	...
	{
		$r = new ReflectionClass(SafeConfigLoader::class);
		$publicMethods = array_map(fn (\ReflectionMethod $m) => $m->getName(), $r->getMethods(\ReflectionMethod::IS_PUBLIC));
	...
	public function testLoadAcceptsFileReturningArrayExpression(): void
	{
		...
		$path = $this->writeTempPhp("<?php\n\$v = 1;\nreturn ['computed' => \$v];", 'computed.php');

		$this->assertSame([
			'computed' => 1
		], SafeConfigLoader::load($path));
	}
}
