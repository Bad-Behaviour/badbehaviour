...
class SafeModeTest extends TestCase
{
	 ...
	 * defaults — not the entire config.
	 * This is what makes the diff
	...
	public function test_overrides_relate_to_safety_features(): void
	{
		...
		$this->assertEmpty($unexpected, 'SafeMode::overrides() contains unexpected keys (not safety-related): ' . implode(', ', $unexpected));
	}

	/**
	 * SafeMode::overrides() must contain exactly the keys we expect
	 * to differ from defaults.
	 * This catches accidental additions or
	...
	public function test_overrides_contains_expected_keys(): void
	{
		...
		foreach ($expected as $key) {
			$this->assertArrayHasKey($key, $overrides, "overrides() must contain '$key'");
		}
	}

	/**
	 * SafeMode::overrides() may contain entries that match defaults —
	 * this is INTENTIONAL for safety-critical features.
	 * Explicit
	...
	public function test_overrides_only_contains_safety_relevant_keys(): void
	{
		...
		$this->assertEmpty($unexpected, 'SafeMode::overrides() contains non-safety keys. ' . 'Remove them: ' . implode(', ', $unexpected));
	}

	/**
	 * settings() must include Configuration::get_defaults() PLUS
	 * overrides() PLUS log_table injection.
	 * Used by Configuration::from_array()
	...
	public function test_settings_includes_defaults_and_overrides(): void
	{
		...
		foreach (array_keys($defaults) as $key) {
			$this->assertArrayHasKey($key, $settings, "settings() must include default key '$key'");
		}

		// log_table injection works
		$this->assertSame('my_log_table', $settings['log_table'], 'settings() must inject the provided log_table');
	}
	...
	public function test_settings_includes_safe_mode_marker(): void
	{
		...
		$this->assertTrue($settings['_safe_mode'], 'settings() must set _safe_mode = true');
	}
	...
	public function test_overrides_win_over_defaults(): void
	{
		...
		$this->assertTrue($defaults['dns_verification']['enabled'], 'precondition: dns_verification.enabled default is true');
		$this->assertFalse($overrides['dns_verification']['enabled'], 'precondition: dns_verification.enabled override is false');
		$this->assertFalse($merged['dns_verification']['enabled'], 'merged value must be override (false), not default (true)');
	}
	...
	public function test_merge_layers_additional_overrides(): void
	{
		...
		$this->assertFalse($merged['logging'], 'adapter override must disable logging');
		$this->assertSame('custom', $merged['preset'], 'adapter override must change preset');
		$this->assertSame('adapter_log', $merged['log_table'], 'adapter override must replace log_table');

		// Safe-mode settings still apply for non-overridden keys
		$this->assertFalse($merged['dns_verification']['enabled'], 'safe-mode dns_verification.enabled must still be false');
		$this->assertTrue($merged['_safe_mode'], 'safe-mode marker must persist through merge');
	}

	/**
	 * forbidden_flat_keys() must list every flat key that would be
	 * silently ignored by Configuration::from_array().
	 * This is the
	...
	public function test_overrides_contains_no_forbidden_flat_keys(): void
	{
		...
		foreach ($forbidden as $flat_key) {
			$this->assertArrayNotHasKey($flat_key, $overrides, "overrides() must not contain forbidden flat key '$flat_key'. " . 'Use nested form (e.g., dns_verification.enabled) instead.');
		}
	}

	/**
	 * settings() must produce a Configuration that survives from_array()
	 * with all defenses off.
	 * End-to-end test of the safe-mode path.
	 */
	public function test_settings_produces_valid_configuration(): void
	{
		...
		$this->assertFalse($config->dns_verification_enabled, 'safe-mode → dns_verification_enabled must be false');
		$this->assertFalse($config->dynamic_ip_ranges_enabled, 'safe-mode → dynamic_ip_ranges_enabled must be false');
		$this->assertFalse($config->reverse_proxy, 'safe-mode → reverse_proxy must be false');
		$this->assertFalse($config->dnsbl_enabled, 'safe-mode → dnsbl_enabled must be false');
		$this->assertTrue($config->logging, 'safe-mode → logging must remain true');
		$this->assertSame('integration_test_log', $config->log_table, 'safe-mode → log_table must be the injected value');
	}

	/**
	 * SafeMode must NOT override performance.skip_extensions.
	 * Safe-mode
	...
	public function test_safemode_preserves_static_skip_defaults(): void
	{
		...
		$this->assertArrayNotHasKey('performance', $overrides, 'SafeMode must not override performance — static-skip must inherit defaults');
		$this->assertArrayHasKey('performance', $defaults, 'precondition: defaults must contain performance');
		$this->assertContains('js', $defaults['performance']['skip_extensions'], 'precondition: defaults skip_extensions must contain js');
	}

	/**
	 * settings() must produce a Configuration where skip_static_extensions
	 * still contains the full default set.
	...
	public function test_safemode_settings_produces_full_static_skip(): void
	{
		...
		$this->assertNotEmpty($config->skip_static_extensions, 'safe-mode Configuration must have non-empty skip_static_extensions');
		$this->assertContains('js', $config->skip_static_extensions, "safe-mode Configuration must have 'js' in skip_static_extensions");
		$this->assertGreaterThanOrEqual(10, count($config->skip_static_extensions), 'safe-mode Configuration must have full default static-skip set');
	}

	/**
	 * SafeMode must keep logging enabled.
	 * Logging is the whole point of
	...
	public function test_safemode_keeps_logging_enabled(): void
	{
		...
		$this->assertArrayNotHasKey('logging', $overrides, 'SafeMode must not disable logging (the whole point is to observe)');
	}
	...
	public function test_safemode_verbose_is_false(): void
	{
		...
		$this->assertFalse($config->verbose, 'safe-mode must not enable verbose logging (would flood log table with asset requests)');
	}
	 ...
	 * when their values match defaults.
	...
	public function test_safety_critical_features_are_explicit(): void
	{
		...
		foreach ($required as $key => $expected_subkeys) {
			$this->assertArrayHasKey($key, $overrides, "SafeMode must explicitly list '$key' even if it matches defaults. " . "Removing it removes safety documentation and future-proofing.");

			foreach ($expected_subkeys as $sub_key => $expected_value) {
				$this->assertArrayHasKey($sub_key, $overrides[$key], "SafeMode['$key'] must contain '$sub_key'");
				$this->assertSame($expected_value, $overrides[$key][$sub_key], "SafeMode['$key']['$sub_key'] must be " . var_export($expected_value, true));
			}
		}
	}
}