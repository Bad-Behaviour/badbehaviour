...
$results['static_css'] = bench('Static CSS (fast path)', function () use ($bb) {
	$bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0 (Windows NT 10.0) Chrome/120', '203.0.113.1', 'GET', '/wp-content/themes/style.css'));
}, 5000);
...
$results['browser_html'] = bench('Browser HTML page', function () use ($bb) {
	$bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36', '203.0.113.1', 'GET', '/about'));
}, 1000);
...
$results['ajax_post'] = bench('AJAX POST (JSON)', function () use ($bb) {
	$bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0 Chrome/120', '203.0.113.1', 'POST', '/api/save', [
		'Host' => 'example.com',
...
$results['googlebot_valid'] = bench('Googlebot (valid IP)', function () use ($bb) {
	$bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)', '66.249.66.1', // In Google's range
	'GET', '/sitemap.xml'));
}, 1000);
...
$results['fake_googlebot'] = bench('Fake Googlebot (UA only)', function () use ($bb) {
	$bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0 (compatible; Googlebot/2.1)', '198.51.100.42', // NOT in Google's range
	'GET', '/admin'));
}, 500);
...
$results['empty_ua'] = bench('Empty UA (fast path)', function () use ($bb) {
	$bb->run_test_package(RequestPackage::create_for_test('', '198.51.100.43', 'GET', '/'));
}, 1000);
...
$results['sqlmap'] = bench('SQLMap UA (blocked)', function () use ($bb) {
	$bb->run_test_package(RequestPackage::create_for_test('sqlmap/1.5.2', '198.51.100.44', 'GET', '/'));
}, 500);
...
$results['curl'] = bench('cURL (http_tool)', function () use ($bb) {
	$bb->run_test_package(RequestPackage::create_for_test('curl/8.4.0', '198.51.100.45', 'GET', '/health'));
}, 1000);
...
$results['cold_dns'] = bench('Cold DNS cache', function () use ($bb, $adapter) {
	...
	$bb->run_test_package(RequestPackage::create_for_test('Mozilla/5.0 (compatible; Googlebot/2.1)', '198.51.100.42', 'GET', '/admin'));
